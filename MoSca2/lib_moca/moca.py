...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track", "segment"], f"{static_id_mode} not in [raft, track, segment]"
    s2d.to(device)
    ...
    # if s2d.has_segment:
    #     segment_mask = s2d.segment_mask.clone().float()
    #     with torch.no_grad():
    #         sta_track_test_dir = osp.join(ws, "track_test")
    #         os.mkdir(sta_track_test_dir)
    #         epierr_list = query_buffers_by_track(segment_mask[..., None] * 3 / (2* segment_mask.shape[0]), track, track_mask)
    #         epierr_list = epierr_list.squeeze(-1).cpu()
    #         track_static_selection, track_dynamic_selection = identify_tracks(
    #             epierr_list, epi_th
    #         )
    #         torch.save(track_static_selection, osp.join(sta_track_test_dir, "threshold=2|3.pth"))
    #         epierr_list = query_buffers_by_track(segment_mask[..., None] * 2 / (segment_mask.shape[0]), track, track_mask)
    #         epierr_list = epierr_list.squeeze(-1).cpu()
    #         track_static_selection, track_dynamic_selection = identify_tracks(
    #             epierr_list, epi_th
    #         )
    #         torch.save(track_static_selection, osp.join(sta_track_test_dir, "threshold=1|2.pth"))
    #         epierr_list = query_buffers_by_track(segment_mask[..., None] / (segment_mask.shape[0]), track, track_mask)
    #         epierr_list = epierr_list.squeeze(-1).cpu()
    #         track_static_selection, track_dynamic_selection = identify_tracks(
    #             epierr_list, epi_th
    #         )
    #         torch.save(track_static_selection, osp.join(sta_track_test_dir, "threshold=1.pth"))

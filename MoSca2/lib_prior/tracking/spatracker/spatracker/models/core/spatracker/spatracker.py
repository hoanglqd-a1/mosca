...
class SpaTracker(nn.Module):
    ...
    def forward_iteration(
        ...
        print("forward", torch.cuda.memory_reserved(device) / 1024**3, "GB")
        coord_predictions = []
        attn_predictions = []
        Rot_ln = 0
        ...
        for __ in range(iters):
            print("forward forward start", torch.cuda.memory_reserved(device) / 1024**3, "GB")
            ...
            #                                                    intrs_S, T_mark)
            with torch.no_grad():
                fcorrsXY = fcorr_fnXY.corr_sample(ffeats[0], coords[..., :2]).cpu()
                fcorrsYZ = fcorr_fnYZ.corr_sample(ffeats[1], coords[..., [1,2]]).cpu()
                fcorrsXZ = fcorr_fnXZ.corr_sample(ffeats[2], coords[..., [0,2]]).cpu()
            # fcorrs = fcorrsXY 
            ...
            flows_cat = flows_cat.to(device)
            flows_cat =  self.zeroMLPflow(flows_cat)
            flows_cat = flows_cat.cpu()
            ...
            print("forward forward", torch.cuda.memory_reserved(device) / 1024**3, "GB")
            ...
            ffeats_xy = self.ffeat_updater(self.norm(d_feats.view(-1, self.latent_dim))).cpu() + ffeats_xy.reshape(-1, self.latent_dim)
            ffeats_yz = self.ffeatyz_updater(self.norm(d_feats.view(-1, self.latent_dim))).cpu() + ffeats_yz.reshape(-1, self.latent_dim)
            ffeats_xz = self.ffeatxz_updater(self.norm(d_feats.view(-1, self.latent_dim))).cpu() + ffeats_xz.reshape(-1, self.latent_dim)
            ...
            coords = coords + d_coord.reshape(B, N, S, 3).permute(0, 2, 1, 3).cpu()
            ...
            print("forward forward end", torch.cuda.memory_reserved(device) / 1024**3, "GB")
        print("forward", torch.cuda.memory_reserved(device) / 1024**3, "GB")

        ffeats_f = ffeats[0] + ffeats[1] + ffeats[2]
        ffeats_f = ffeats_f.to(device)
    ...
    def forward(self, rgbds, queries, iters=4, feat_init=None,
        ...
        while w_idx_start < T - self.S // 2:
            print("Start tracking with S = ", self.S)
            print(torch.cuda.memory_reserved(device) / 1024**3, "GB")  
            ...
            fmapYZ = fmapYZ.cpu()
            fmapXZ = fmapXZ.cpu()
            feat_init = feat_init.cpu()
            vis_init_ = vis_init_.cpu()
            coords_init_ = coords_init_.cpu()
            track_mask = track_mask.cpu()
            self.intrs = self.intrs.cpu()
            print(torch.cuda.memory_allocated(device) / 1024 ** 3, "GB")
            ...
            print(torch.cuda.memory_allocated(device) / 1024 ** 3, "GB")
            fmapXY = fmapXY.to(device)
            fmapYZ = fmapYZ.to(device)
            fmapXZ = fmapXZ.to(device)
            feat_init = feat_init.to(device)
            vis_init_ = vis_init_.to(device)
            coords_init_ = coords_init_.to(device)
            track_mask = track_mask.to(device)
            self.intrs = self.intrs.to(device)

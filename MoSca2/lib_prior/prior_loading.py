...
class Saved2D(nn.Module):
    ...
    def load_segment(self, segment_dirname="segment_mask"):
        segment_dir = osp.join(self.ws, segment_dirname)
        segment_lst = []
        file_paths = sorted(glob(osp.join(segment_dir, "*.npy")))
        for path in file_paths:
            mask = np.load(path)
            segment_lst.append(mask)
        segment_mask = torch.from_numpy(np.stack(segment_lst)).float()

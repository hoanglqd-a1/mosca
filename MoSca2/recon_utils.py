...
def update_s2d_track_identification(
    ...
    print("track shape", s2d.track.shape)
    if s2d.has_segment:
        segment_mask = s2d.segment_mask.clone()
        print("Segment mask shape[0]:", segment_mask.shape[0])
        with torch.no_grad():
            epi_error_list = query_buffers_by_track(
                segment_mask[..., None] / segment_mask.shape[0], s2d.track, s2d.track_mask
            )
...
def set_epi_mask_to_s2d_for_bg_render(s2d, epi_th, device):
    ...
    return s2d
def set_segment_mask_to_s2d_for_bg_render(s2d, device):
    assert s2d.has_segment, "SEGMENT is required for static warm"
    static_mask = s2d.segment_mask < 0.5
    # erode the static mask
    kernel = torch.ones((3, 3), device=device)
    static_mask = kornia.morphology.erosion(
        static_mask.float().unsqueeze(1), kernel

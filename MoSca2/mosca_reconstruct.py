...
from recon_utils import (
    ...
    set_segment_mask_to_s2d_for_bg_render,
)
...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    s2d = set_segment_mask_to_s2d_for_bg_render(s2d, device)
    ...
    print("static rgb shape", rgb.shape)
    print("Save path: " + osp.join(log_path, "photo_warmup_rendered.npz"))
    np.savez(
        osp.join(log_path, "photo_warmup_rendered.npz"),
        rgb=rgb.cpu().numpy(),
        dep=dep.cpu().numpy(),
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    if s2d.has_segment:
        viz_segment_mask = s2d.segment_mask > 0.5
        viz_segment_mask = viz_segment_mask[..., None] * s2d.rgb
        imageio.mimsave(
            osp.join(log_path, f"segment_hardmask.gif"),
            (viz_segment_mask.cpu().numpy() * 255).astype(np.uint8),
        )
    cams: MonocularCameras = MonocularCameras.load_from_ckpt(
        torch.load(osp.join(log_path, "bundle", "bundle_cams.pth"))
    ).to(device)
    ...
    if refilter_curve_by_photo_error_cnt > 0:
        print("refilter_curve_by_photo_error_cnt", refilter_curve_by_photo_error_cnt)
        photo_error_masks = get_static_render_error_mask(
            s2d,
            log_path,
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    c_model = ColorModule(getattr(fit_cfg, "gs_max_sph_order", 0)).to(device)

    photo_solver.lightning_fit(
        s2d,
        cams,
        c_model,
        s_model,

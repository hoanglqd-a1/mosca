...
from lib_mosca.color import ColorModule
...
class DynReconstructionSolver:
    ...
    def photometric_fit(
        ...
        for step in tqdm(range(total_steps)):
            ...
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                ...
                if c_model:
                    ...
                    s_xyz = gs5[0][0]
                    with torch.no_grad():
                        _, cam_pos = cams.Rt_wc(view_ind)
                        direction = s_xyz - cam_pos[None].to(s_xyz.device)
                        direction = F.normalize(direction, dim=-1)
                    gs5[0][-1] = c_model(torch.cat([s_sph, direction, s_xyz], -1).to(s_sph.device))
                ...
                if d_flag:
                    ...
                    if c_model:
                        ...
                        with torch.no_grad():
                            _, cam_pos = cams.Rt_wc(view_ind)
                            direction = d_xyz - cam_pos[None].to(d_xyz.device)
                            direction = F.normalize(direction, dim=-1)
                        gs5[1][-1] = c_model(torch.cat([d_sph, direction, d_xyz], -1))
        ...
        if c_model:
            torch.save({"state_dict": c_model.state_dict(),
                    "max_sh_degree": c_model.max_sh_degree},
                   osp.join(self.log_dir, f"{phase_name}_color.pth"))
    ...
    def lightning_fit(
        ...
        for step in range(iterations):
            ...
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                with torch.no_grad():
                    gs5 = [list(s_model()), list(d_model(view_ind))]
                    s_sph = gs5[0][-1]
                    s_xyz = gs5[0][0]
                    _, cam_pos = cams.Rt_wc(view_ind)
                    s_direction = s_xyz - cam_pos[None].to(s_xyz.device)
                    s_direction = F.normalize(s_direction, dim=-1)

                    d_sph = gs5[1][-1]
                    d_xyz = gs5[1][0]
                    d_direction = d_xyz - cam_pos[None].to(d_xyz.device)
                    d_direction = F.normalize(d_direction, dim=-1)

                gs5[0][-1] = c_model(torch.cat([s_sph, s_direction, s_xyz], -1)).to(s_sph.device)
                gs5[1][-1] = c_model(torch.cat([d_sph, d_direction, d_xyz], -1)).to(d_xyz.device)

                add_buffer = None
                dst_xyz = torch.cat([gs5[0][0].detach(), d_model(view_ind)[0]], 0)
                dst_xyz_cam = cams.trans_pts_to_cam(view_ind, dst_xyz)
                if GS_BACKEND in ["native_add3"]:
                    add_buffer = dst_xyz_cam

                render_dict = render(gs5, s2d.H, s2d.W, cams.K(s2d.H, s2d.W), cams.T_cw(view_ind), bg_color=[1.0, 1.0, 1.0], add_buffer=add_buffer)

                rgb_sup_mask = s2d.get_mask_by_key("all")[view_ind]
                _l_rgb, _, _, _ = compute_rgb_loss(s2d.rgb[view_ind].detach().clone(), render_dict, rgb_sup_mask)
        ...
        torch.save({"state_dict": c_model.state_dict(),
                    "max_sh_degree": c_model.max_sh_degree},
                    osp.join(self.log_dir, "photometric_color.pth"))
        return

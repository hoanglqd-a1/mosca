...
from lib_mosca.featured_dynamic_gs import FeaturedDynamicGS
...
def test_main(
    ...
    #     device=device,
    # )
    feature_scf = FeaturedMosca.load_from_ckpt(
        torch.load(osp.join(saved_dir, "mosca", "featured_mosca.pth"))
    )
    deformation_net = DeformationNet.load_from_ckpt(
        torch.load(osp.join(saved_dir, "mosca", "deformation.pth"))
    )
    d_model = FeaturedDynamicGS.load_from_ckpt(
        torch.load(osp.join(saved_dir, f"photometric_d_model_{GS_BACKEND.lower()}.pth")),
        feature_scf,
        deformation_net
    )

    feature_scf.to(device)
    feature_scf.eval()
    deformation_net.to(device)

...
from lib_mosca.dynamic_solver import geometry_scf_init, featured_scf_init
from lib_mosca.photo_recon_utils import OptimCFG, GSControlCFG
from lib_mosca.mosca import MoSca
from lib_mosca.featured_mosca import FeaturedMosca
from lib_mosca.deformation_net import DeformationNet
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    deformation_net = DeformationNet(
        ...
        spherical_harmonics_degree=getattr(fit_cfg, "gs_max_sph_order", 0)
    )
    featured_scaffold = FeaturedMosca(
        scaffold,
        FEATURE_DIM,
    )
    device = "cuda"
    featured_scf_init(
        log_dir=osp.join(log_path, "featured_mosca"),
        featured_scf=featured_scaffold,
        deformation_net=deformation_net,
        device=device,
    )
    test_featured_scf = [deformation_net(featured_scaffold.node_feature, tid)[0] for tid in range(cams.T)]
    viz_list = viz_list_of_colored_points_in_cam_frame(
        [cams.trans_pts_to_cam(t, it).cpu() for t, it in enumerate(test_featured_scf)],
        node_rgb,
        zoom_out_factor=1.0,
    )
    imageio.mimsave(osp.join(log_path, "featured_cam_curve_optimized.gif"), viz_list, loop=1000)

    os.makedirs(osp.join(log_path, "mosca"), exist_ok=True)
    torch.save(scaffold.state_dict(), osp.join(log_path, "mosca", "mosca.pth"))
    # torch.save(featured_scaffold.state_dict(), osp.join(log_path, "mosca", "featured_mosca.pth"))
    torch.save(featured_scaffold.state_dict(), osp.join(log_path, "mosca", "pretrained_featured_mosca.pth"))
    torch.save(deformation_net.state_dict(), osp.join(log_path, "mosca", "deformation.pth"))
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    # logging.info("Memory allocated after loading s2d:", torch.cuda.memory_allocated(device) / 1024 ** 2, "MB")
    ...
    # logging.info("Memory allocated after loading cams and scaffold:", torch.cuda.memory_allocated(device) / 1024 ** 2, "MB")
    ...
    featured_scf = FeaturedMosca.load_from_ckpt(
        torch.load(osp.join(log_path, "mosca", "pretrained_featured_mosca.pth"))
    )
    deformation_net = DeformationNet.load_from_ckpt(
        torch.load(osp.join(log_path, "mosca", "deformation.pth"))
    )
    ...
    # logging.info("Memory allocated after loading photo solver:", torch.cuda.memory_allocated(device) / 1024 ** 2, "MB")
    ...
    logging.info(f"Memory allocated after loading static model: {torch.cuda.memory_allocated(device) / 1024 ** 2} MB")
    ...
    # f_d_model = photo_solver.get_featured_dynamic_model(
    #     s2d=s2d,
    #     cams=cams,
    #     featured_scf=featured_scf,
    #     deformation_net=deformation_net,
    #     n_init=getattr(fit_cfg, "gs_dynamic_n_init", 30000),
    #     additional_mask=d_model_add_mask,
    #     max_sph_order=getattr(fit_cfg, "gs_max_sph_order", 0),
    # )
    logging.info(f"Memory allocated after loading dynamic model: {torch.cuda.memory_allocated(device) / 1024 ** 2} MB")
    with torch.no_grad():
    ...
    # )
    photo_solver.featured_photometric_fit(
        s2d=s2d,
        total_steps=1000,
        optim_cam_after_steps=getattr(fit_cfg, "photo_optim_cam_after_steps", 0),
        decay_start=getattr(fit_cfg, "photo_decay_start", 2000),
...
if __name__ == "__main__":
    ...
    logdir = setup_recon_ws(args.ws, fit_cfg=cfg, reuse_log_flag=True)

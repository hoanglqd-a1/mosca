...
from featured_dynamic_gs import FeaturedDynamicGS
...
class DynReconstructionSolver:
    ...
    def get_featured_dynamic_model(
        self,
        s2d: Saved2D,
        cams: MonocularCameras,
        ...
        unique_tid = time_init.unique()
        logging.info("Attach to Featured Dynamic Scaffold ...")
        for tid in tqdm(unique_tid):
            t_mask = time_init == tid
            f_d_model.append_new_gs(
                tid=tid,
                xyz=mu_init[t_mask],
                quat=q_init[t_mask],
                scale=s_init[t_mask],
                opacity=o_init[t_mask],
                rgb=rgb_init[t_mask],
            )
        return f_d_model
    ...
    def photometric_fit(
        ...
        # prepare a color-plate for semantic rendering
        if d_flag:
    ...
    def featured_photometric_fit(
        ...
        if len(cam_param_list) > 0:
            ...
            optimizer_cam = None
        optimizer_scf = torch.optim.Adam(
            f_d_model.featured_scf.parameters(), lr=0
        )
        optimizer_d_model = torch.optim.Adam(
            f_d_model.parameters(), lr=2e-5
        )
        optimizer_deformation = torch.optim.Adam(
            deformation_net.parameters(), lr=3e-6,
        )
        if use_decay:
            gs_scheduling_func_dict, cam_scheduling_func_dict = (
                optimizer_cfg.get_scheduler(total_steps=total_steps - decay_start)
        ...
        n_group_static = len(s_model.group_id.unique())
        color_plate = get_colorplate(n_group_static)
        # random permute
        color_permute = torch.randperm(len(color_plate))
        color_plate = color_plate[color_permute]
        s_model.get_cate_color(
        ...
        for step in tqdm(range(total_steps)):
            ...
            cams.zero_grad()
            s_model.zero_grad()
            optimizer_scf.zero_grad()
            optimizer_deformation.zero_grad()
            s2d.zero_grad()
            optimizer_d_model.zero_grad()
            if step > decay_start:
                for k, v in gs_scheduling_func_dict.items():
                    update_learning_rate(v(step), k, optimizer_static)
                if optimizer_cam is not None:
                    for k, v in cam_scheduling_func_dict.items():
                        update_learning_rate(v(step), k, optimizer_cam)
            view_ind_list = np.random.choice(cams.T, n_frame, replace=False).tolist()
            # view_ind_list = np.arange(cams.T).tolist()
            ...
            loss_mask = 0.0
            loss_track = 0.0
            for inner_loop_i, view_ind in enumerate(view_ind_list):
                gs5 = [list(s_model())]
                gs5.append(list(f_d_model(view_ind)))
                ...
                cannonical_mask = f_d_model.ref_time == view_ind
                dep_sup_mask = rgb_sup_mask * s2d.dep_mask[view_ind]
                _l_dep, _, _, _ = compute_dep_loss(
                    s2d.dep[view_ind].detach().clone(),
                    render_dict,
        ...
        # save static, camss, featured_scf, deformation, dynamic gs
        torch.save(s_model.state_dict(), s_save_fn)
        torch.save(cams.state_dict(), osp.join(self.log_dir, f"{phase_name}_cam.pth"))
        torch.save(f_d_model.featured_scf.state_dict(), osp.join(self.log_dir, "mosca", "featured_mosca.pth"))
        torch.save(deformation_net.state_dict(), osp.join(self.log_dir, "mosca", "deformation.pth"))
        fd_save_fn = osp.join(
            self.log_dir, f"{phase_name}_d_model_{GS_BACKEND.lower()}.pth"
        )
        torch.save(f_d_model.state_dict(), fd_save_fn)
        torch.cuda.empty_cache()
        return

...
def make_viz_graph(d_model, view_ind, cams, view_cam_id=None, max_radius=0.001):
    # node_mu_w = d_model.scf.node_xyz[d_model.get_tlist_ind(view_ind)]
    node_mu_w = d_model.featured_scf.node_xyz[view_ind] # this is changed to be suitable with featured dynamic gs
    if view_cam_id is None:
...
def viz2d_flow_video(
    ...
    render_dict = render(
        [s_model(active_sph_order=0), d_model(start_from, 0, nn_fusion=-1)],
        H,
        W,
        cams.default_K,
...
def viz3d_total_video(
    ...
    frames_node = viz_curve(
        d_model.scf.node_xyz,
        torch.ones_like(d_model.scf.node_xyz[..., 0]).bool(),
        cams,
        res=res,
        text="All ED Nodes",
        viz_n=-1,

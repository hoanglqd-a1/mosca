...
class DynSCFGaussian(nn.Module):
    ...
    def get_group(self):
        ...
        group_id = self.scf.node_grouping[self.attach_ind]
        return group_id
    ...
    def __check_uncovered_mu__(self, mu_w, tid):
        node_xyz_live = self.scf.node_xyz[self.get_tlist_ind(tid)]
    ...
    def append_new_gs(self, optimizer, tid, mu_w, quat_w, scales, opacity, rgb):
        ...
        _, attach_ind, _ = knn_points(mu_w[None], self.scf.node_xyz[tid][None], K=1)
        ...
        if self.leaf_local_flag:
            attach_node_xyz = self.scf.node_xyz[tid][attach_ind[0, :, 0]]
            attach_node_R_wi = q2R(self.scf.node_rotation[tid][attach_ind[0, :, 0]])
    ...
    def append_new_node_and_gs(
        ...
        _, attach_ind, _ = knn_points(
            mu_w[None], self.scf.node_xyz[tid][old_M:][None], K=1
        )
    ...
    def gradient_based_node_densification(
        ...
        self.scf.append_nodes_traj(
            ...
            new_node_xyz,
            new_node_quat,
            torch.zeros(new_node_xyz.shape[1]).to(self.scf.node_grouping),
        )

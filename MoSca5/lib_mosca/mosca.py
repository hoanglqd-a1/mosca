...
class MoSca(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "node_certain", node_certain.to(init_device)
        )  # if certain True
        self.register_buffer("node_grouping", node_grouping.to(init_device))
        self.register_buffer("unique_grouping", torch.unique(self.node_grouping))
        ...
        if spatial_unit_hard_set is None or spatial_unit_hard_set <= 0.0:
            ...
            spatial_unit = spatial_unit_factor * __identify_spatial_unit_from_curves__(
                self.node_xyz, self.node_certain, K=skinning_k
            )
    ...
    def load_from_ckpt(
        ...
        node_certain = ckpt["node_certain"]
        node_grouping = ckpt["node_grouping"]
    ...
    def update_topology(
        ...
        if len(self.node_grouping.unique()) != len(self.unique_grouping):
            print()
        assert len(self.node_grouping.unique()) == len(
    ...
    def fast_warp(
        ...
        sk_dst_node_quat = self.node_rotation[target_tid][sk_ind]
        ret_xyz, ret_dir = self.__BLEND_FUNC__(
            sk_w=sk_w,
            src_xyz=query_xyz,
            src_R=query_dir,
    ...
    def append_nodes_pnt(
        ...
        while len(new_node_xyz) > 0:
            ...
            new_group_id = self.node_grouping[nn_exsiting_ind]
    ...
    def append_nodes_traj(
        ...
        self.node_certain = torch.cat(
            [self.node_certain, append_valid_mask], 1
        )  # T,M'
    ...
    def compute_vel_acc_loss(
        ...
        xyz = self.node_xyz[tids]
        R_wi = q2R(self.node_rotation[tids])
                ...
                for gid in unqiue_query_id:
                    ...
                    if mask_gid.any():
                        ...
                        if not node_mask.any():
                            ...
                            # )
                            node_mask = torch.ones_like(self.node_grouping).bool()

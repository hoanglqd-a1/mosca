...
def line_segment_init(track_mask, point_ref):
    ...
    inverse_muti = torch.arange(T, 0, -1, device=working_point_ref.device, dtype=working_point_ref.dtype)
...
def geometry_scf_init(
    ...
    for step in tqdm(range(total_steps)):
        if step in reline_steps:
            ...
            new_xyz = line_segment_init(scf.node_certain, scf.node_xyz.detach())
            with torch.no_grad():
                scf.node_xyz.data = new_xyz
            ...
            if viz_node_rgb is not None:
                viz_list = viz_list_of_colored_points_in_cam_frame(
                    [
                        ...
                        for t, it in enumerate(scf.node_xyz)
                    ],
                    viz_node_rgb,
                    zoom_out_factor=1.0,
                )
                    ...
                    for level in [-1] + [
                        ...
                        viz_list = viz_list_of_colored_points_in_cam_frame(
                            [
                                ...
                                for t, it in enumerate(scf.node_xyz.detach())
                            ],
...
def featured_scf_init(
    ...
    featured_scf = featured_scf.to(device)
    deformation_net = deformation_net.to(device)

    optimizer = torch.optim.Adam(
        [
            {"params": featured_scf.parameters(), "lr": 1e-3},
            {"params": deformation_net.parameters(), "lr": 1e-3}
        ]
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
    ...
    batch_size = featured_scf.node_feature.shape[0]
    t_list = featured_scf.t_list

    from tqdm import trange
    for step in trange(total_steps):
        optimizer.zero_grad()
        # Batch all view indices
        view_idx_tensor = t_list.repeat_interleave(batch_size)
        node_feature_batch = featured_scf.node_feature.repeat(len(t_list), 1)
        xyz_pred, _, _, _, _ = deformation_net(node_feature_batch, view_idx_tensor.unsqueeze(1), flag=False)
        xyz_gt = torch.cat([featured_scf.node_xyz[view_idx] for view_idx in t_list], dim=0)
        xyz_loss = mean_squared(xyz_pred, xyz_gt)
        loss = xyz_loss * lambda_xyz
        loss.backward()
        optimizer.step()
        if step > decay_start:
            scheduler.step()

...
def line_segment_init(track_mask, point_ref):
    ...
    track_mask_valid_cnt = track_mask.sum(0)
    working_mask = track_mask_valid_cnt > 0
    logging.info(f"Line Segment Init, invalid curve cnt={(~working_mask).sum()}")

    # filter out the empty track
    ...
    inverse_muti = torch.arange(T, 0, -1, dtype=working_point_ref.dtype, device=working_point_ref.device)
...
def geometry_scf_init(
    ...
    scf.xyz_cannon.data = scf._node_xyz.data[0].clone()
    return scf

def feature_scf_init(
    log_dir,
    scf: MoSca,
    device,
    total_steps=8000,
    ...
    deformation = scf.deformation
    inversed_deformation = scf.inverse_deformation

    scf = scf.to(device)
    optimizer = torch.optim.Adam(
        deformation.parameters(), lr=1e-3
    )
    optimizer_inversed = torch.optim.Adam(
        inversed_deformation.parameters(), lr=1e-3
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        (total_steps - decay_start),
    ...
    scheduler_inversed = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_inversed,
        (total_steps - decay_start),
        eta_min=1e-6,
    )
    N = scf.xyz_cannon.shape[0]
    T = scf.T

    # with torch.no_grad():
    #     t_list = scf._t_list[1:]
    #     view_idx_tensor = t_list.repeat_interleave(N).unsqueeze(1)
    #     xyz_1_to_n = torch.cat([scf._node_xyz[view_idx] for view_idx in t_list], dim=0)
    #     batch_cannonical_xyz = scf.xyz_cannon.repeat(len(t_list), 1)

    for step in tqdm(range(total_steps)):
        optimizer_inversed.zero_grad()
        time_idx = (torch.randperm(T-1) + 1)[:time_batch_size].to(scf.device) # (B,)
        deformed_xyz = scf._node_xyz[time_idx].reshape(time_batch_size * N, 3) # (B * N, 3)
        time_idx = time_idx.repeat_interleave(N).unsqueeze(1) # (B * N, 3)
        batch_cannonical_xyz = scf.xyz_cannon.repeat(time_batch_size, 1) # (B * N, 3)
        inversed_xyz_pred = inversed_deformation(deformed_xyz, time_idx)
        inversed_xyz_loss = torch.nn.functional.mse_loss(inversed_xyz_pred, batch_cannonical_xyz)
        loss = inversed_xyz_loss
        loss.backward()
        if step > decay_start:                    
            scheduler_inversed.step()
        if step % 100 == 0:
            with open("logs.txt", "a") as f:
                f.write(f"loss inversed xyz = {loss.item()}\n")
                # f.write(f"loss inversed xyz = {inversed_xyz_loss.item()}\n")
            print(f"loss reversed xyz = {loss.item()}")
        optimizer_inversed.step()

    for step in tqdm(range(total_steps)):
        optimizer.zero_grad()
        time_idx = (torch.randperm(T-1) + 1)[:time_batch_size].to(scf.device)
        deformed_xyz = scf._node_xyz[time_idx].reshape(time_batch_size * N, 3)
        time_idx = time_idx.repeat_interleave(N).unsqueeze(1)
        batch_cannonical_xyz = scf.xyz_cannon.repeat(time_batch_size, 1)
        xyz_pred = deformation(batch_cannonical_xyz, time_idx)
        xyz_loss = torch.nn.functional.mse_loss(xyz_pred, deformed_xyz)

        # inversed_xyz_pred = inversed_deformation(xyz_1_to_n, view_idx_tensor)
        # inversed_xyz_loss = torch.nn.functional.mse_loss(inversed_xyz_pred, batch_cannonical_xyz)

        # loss = xyz_loss + inversed_xyz_loss
        loss = xyz_loss
        loss.backward()
        ...
        if step % 100 == 0:
            with open("logs.txt", "a") as f:
                f.write(f"loss forward xyz = {xyz_loss.item()}\n")
                # f.write(f"loss inversed xyz = {inversed_xyz_loss.item()}\n")
            print(f"loss xyz = {xyz_loss.item()}")
        optimizer.step()

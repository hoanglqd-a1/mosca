...
from lib_mosca.deformation import Deformation
...
class MoSca(nn.Module):
    # node_xyz: 3d curve
    # node_certain: curve mask
    # _D_topo: # distance matrix of the curve, T,M,M
    deform_flag = False
    def __init__(
        self,
        node_xyz,  # T,M,3
        ...
        # Topology
        skinning_k: int = 8, # number of nearest neighbors
        ...
        # first identify the spatial unit = median distance of the curve
        ...
        ) # set self.mlevel_list, mlevel_k_list, mlevel_w_list
        ...
        # compute the node's distance matrix and compute the topology
        ...
        self.inverse_deformation = Deformation()
        self.register_buffer("xyz_cannon", self._node_xyz.data[0])
        self.summary()
        return
    ...
    def load_from_ckpt(
        ...
        deformation_ckpt = {
            k.replace("deformation.", ""): v
            for k, v in ckpt.items()
            if k.startswith("deformation.")
        }
        deformation = Deformation.load_from_ckpt(
            deformation_ckpt
        )
        scf.load_state_dict(ckpt, strict=True)
        scf.set_skinning_method()
        scf.deformation = deformation
        scf.deform_flag = True
        scf.to(device)
        return scf
    ...
    def decremental_topology(
        self,
        node_prune_mask,  # if prune, mark as True
        verbose=False,
    ...
    def update_topology(
        ...
        if len(unique_group_id) == 1 or not self.break_topo_between_group:
            ...
            for gid in tqdm(unique_group_id):
                ...
                # compute the distance between nodes in the same group
    ...
    def update_multilevel_arap_topo(self, verbose=False):
        if not self.mlevel_arap_flag:
            return
        topo_th = self.topo_th_ratio * self.spatial_unit
        # compute the multilevel topology, return the edge list and corresponding distance
    ...
    def resample_node(self, resample_factor=1.0, use_mask=False, resample_ind=None):
        ...
        new_xyz_cannon = self.xyz_cannon[resample_ind]
        with torch.no_grad():
            if isinstance(self._node_xyz, nn.Parameter):
                self._node_xyz.data = new_node_xyz
            else:
                self._node_xyz = new_node_xyz
            ...
            self.xyz_cannon = nn.Parameter(new_xyz_cannon)
            if len(self._node_grouping.unique()) < len(self.unique_grouping):
                logging.warning(
    ...
    def remove_nodes(self, optimizer, node_prune_mask):
        ...
        spec = ["xyz_cannon", "node_rotation", "node_sigma"]
    ...
    def get_optimizable_list(
        ...
        if lr_cnp is not None:
            ret.append(
                {
                    "params": [self.xyz_cannon],
                    "lr": lr_cnp,
                    "name": "xyz_cannon",
                }
            )
    ...
    def update_node_xyz_quat(self):
        t_list = self._t_list[1:]
        node_cnt = self.xyz_cannon.shape[0]
        view_idx_tensor = t_list.repeat_interleave(node_cnt)
        xyz_pred = self.deformation(self.xyz_cannon, view_idx_tensor.unsqueeze(1), all_view_flag=True, num_time_steps=len(t_list))
        xyz_pred = torch.reshape(xyz_pred, (len(t_list), node_cnt, -1))
        # quat_pred = torch.reshape(quat_pred, (len(self._t_list), node_cnt, -1))
        xyz0 = self.xyz_cannon.unsqueeze(dim=0)
        xyz_pred = torch.cat([xyz0, xyz_pred], dim=0)
        # assert quat_pred.shape == self._node_rotation.shape, f"Misaligned quat shape {quat_pred.shape} vs {self._node_rotation.shape}"
        self._node_xyz = xyz_pred
        # self._node_rotation = quat_pred

    def remove_param_node_xyz(self):
        # Remove it from parameter registry if it was a Parameter
        device = self.device
        if "_node_xyz" in self._parameters:
            tensor = self._parameters.pop("_node_xyz").data.to(device)
            self._node_xyz = tensor  # Now it's just a plain tensor
        else:
            raise RuntimeError("_node_xyz is not a Parameter")
        
    def fix_node_xyz(self):
        data = self._node_xyz
        del self._node_xyz
        self._node_xyz = nn.Parameter(data)
...
def _compute_curve_topo_dist_(
    ...
    # this code generates all pairs 0 <= i <= j < N
    ...
    while cur < P:
        ...
        # compute the distance between src and dst and choose the top_k longest distance
        d = robust_curve_dist_kernel(src, dst, src_m, dst_m, top_k)

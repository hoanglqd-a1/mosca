...
from epi_helpers import analyze_track_epi, identify_epi_tracks, identify_segment_tracks
...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track", "segment", "raft+segment"], f"{static_id_mode} not in [raft, track, segment]"
    s2d.to(device)
    ...
    epierr_list = None
    segment_list = None
    if s2d.has_epi or static_id_mode == "raft":
    ...
    if s2d.has_segment:
        logging.info(f"Use pre-computed 2D segment mask to mark static tracks")
        segment = s2d.segment_mask.clone()
        with torch.no_grad():
            segment_list = query_buffers_by_track(segment[..., None], track, track_mask)
            segment_list = segment_list.squeeze(-1).cpu()
            
    epi_track_static_selection, epi_track_dynamic_selection = None, None
    segment_track_static_selection, segment_track_dynamic_selection = None, None
    epi_track_static_selection, epi_track_dynamic_selection = identify_epi_tracks(
        epierr_list, epi_th
    )
    segment_track_static_selection, segment_track_dynamic_selection = identify_segment_tracks(
        segment_list, static_cnt=track.shape[0], dynamic_cnt=32
    )
    if static_id_mode == "raft" or static_id_mode == "track":
        track_static_selection = epi_track_static_selection
        track_dynamic_selection = epi_track_dynamic_selection
    elif static_id_mode == "segment":
        track_static_selection = segment_track_static_selection
        track_dynamic_selection = segment_track_dynamic_selection
    else:
        # * combine the two selection
        track_static_selection = epi_track_static_selection & segment_track_static_selection
        track_dynamic_selection = epi_track_dynamic_selection & segment_track_dynamic_selection


    # static_track_mask, dynamic_track_mask = track_static_selection, track_dynamic_selection

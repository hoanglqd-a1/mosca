...
def identify_epi_tracks(epierr_list, epi_th, static_cnt=1, dynamic_cnt=1):
    over_th_cnt = (epierr_list > epi_th).sum(0)
    dym_mask = over_th_cnt >= dynamic_cnt
    sta_mask = over_th_cnt < static_cnt
    logging.warning(
        f"Epi Identify {sta_mask.sum()} static, {dym_mask.sum()} dynamic outof {len(sta_mask)} tracks"
    )
    return sta_mask, dym_mask

def identify_segment_tracks(segment_mask, static_cnt=1, dynamic_cnt=1):
    """
    Identify static and dynamic tracks based on segment mask.
    :param segment_mask: (T, N) bool tensor, where True means the track is valid in that frame.
    :param static_cnt: int, number of frames to be considered as static.
    :param dynamic_cnt: int, number of frames to be considered as dynamic.
    :return: static_mask, dynamic_mask
    """
    total_static_cnt = (segment_mask == 0).sum(0)
    total_dynamic_cnt = (segment_mask == 1).sum(0)
    dym_mask = total_dynamic_cnt >= dynamic_cnt
    sta_mask = total_static_cnt >= static_cnt
    logging.warning(
        f"Segment Identify {sta_mask.sum()} static, {dym_mask.sum()} dynamic outof {len(sta_mask)} tracks"
    )
    return sta_mask, dym_mask
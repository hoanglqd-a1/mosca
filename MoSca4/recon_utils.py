...
from lib_moca.epi_helpers import analyze_track_epi, identify_epi_tracks, identify_segment_tracks
...
def setup_recon_ws(ws, fit_cfg, logdir="logs", reuse_log_flag = False):
    seed_everything(SEED)
    if (reuse_log_flag):
        log_dirs = sorted(os.listdir(osp.join(ws, logdir)))
        return osp.join(ws, logdir, log_dirs[-1])
    # get datetime
    now = datetime.now()
...
def update_s2d_track_identification(
    ...
    if s2d.has_segment:
        segment_mask = s2d.segment_mask.clone()
        with torch.no_grad():
            segment_list = query_buffers_by_track(
                segment_mask[..., None], s2d.track, s2d.track_mask
            )
            print(f"Segment list shape: {segment_list.shape}")
    ...
    if method == "raft" or method == "track":
        track_static_selection, track_dynamic_selection = identify_epi_tracks(
            epi_error_list, epi_th, static_cnt=1, dynamic_cnt=dyn_id_cnt
        )
    elif method == "segment":
        track_static_selection, track_dynamic_selection = identify_segment_tracks(
            segment_list, static_cnt=static_id_cnt, dynamic_cnt=dyn_id_cnt
        )
    elif method == "raft+segment":
        ...
        track_static_selection = (
            epi_track_static_selection & segment_track_static_selection
        )
        track_dynamic_selection = (
            epi_track_dynamic_selection & segment_track_dynamic_selection
        )
    ...
    if photo_error_masks is not None:
        ...
        if photo_error_mode == "only":
            track_static_selection = photo_track_static_selection
            track_dynamic_selection = photo_track_dynamic_selection
    ...
    if track_dynamic_selection.sum() < min_curve_num:
        logging.warning(
        ...
        track_dynamic_selection[highest_k] = True

    s2d.register_track_indentification(
        track_static_selection, track_dynamic_selection
    ) # static_track_mask, dynamic_track_mask

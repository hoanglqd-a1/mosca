...
from lib_moca.epi_helpers import analyze_track_epi, identify_epi_tracks
...
def preprocess(
    ...
    #     )
    #     track_static_selection, _ = identify_epi_tracks(epierr_list, EPI_TH)
    ...
    if s2d.has_segment:
        segment_sample_mask = s2d.segment_mask
        segment_sample_mask = segment_sample_mask == 1
        imageio.mimsave(
            osp.join(ws, "segment_resample_mask.gif"),
            segment_sample_mask.cpu().numpy().astype(np.uint8) * 255,
        )

        moca_processor.compute_tap(
            ws=ws,
            save_name=f"segment_dynamic_dep={moca_processor.dep_mode}",
            # n_track=8192 * 3,
            n_track=getattr(pre_cfg, "n_track_pdynamic", 8192 * 3 - 1024),
            img_list=img_list,
            mask_list=segment_sample_mask.detach().cpu().numpy() > 0,
            dep_list=moca_processor.load_dep_list(
                ws, f"{moca_processor.dep_mode}{DEPTH_DIR_POSTFIX}"
            ),
            # K=cams.default_K.detach().cpu().numpy(), # ! maintain the same K as the first infered static one

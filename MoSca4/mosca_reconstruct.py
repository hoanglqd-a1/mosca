...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    s2d = (
        ...
        .load_track(
            f"*segment*{TAP_MODE}", min_valid_cnt=getattr(fit_cfg, "tap_loading_min_valid_cnt", 4)
        )
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    logging.info("scaffold reconstruct stage")
    logging.info("Memory allocated: {}".format(torch.cuda.memory_allocated()))
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg)
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0)
    ...
    s2d = (
        ...
        .recompute_dep_mask(depth_boundary_th=DEPTH_BOUNDARY_TH)
        .load_track(
            f"{TAP_MODE}", min_valid_cnt=getattr(fit_cfg, "tap_loading_min_valid_cnt", 4)
        )
        .rescale_perframe_depth_from_bundle(
            bundle_pth_fn=osp.join(log_path, "bundle", "bundle.pth")
        )
    ...
    logging.info("Memory allocated after load saved2d: {} MB".format(torch.cuda.memory_allocated() / 1024**2))
    ...
    s2d = update_s2d_track_identification(
        ...
        "segment",
        EPI_TH,
        static_id_cnt=s2d.rgb.shape[0],
        dyn_id_cnt=DYN_ID_CNT,
    ...
    # * get scaffold
    # curve_xyz is tracking lifted to 3D, curve_mask is the mask of the curve, curve_rgb is the rgb of the curve
    # node_xyz is the 3D position of mosca nodes
    ...
    # Here we init identity rotation matrix for each mosca's node
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    print("photometric reconstruct stage")
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg)
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0)
    DEP_MEDIAN = getattr(fit_cfg, "dep_median", 1.0)
    ...
    photo_solver.identify_fg_mask_by_segment(
        s2d, "gs_model_construct_segmented_fg_mask.mp4"
    )
...
if __name__ == "__main__":
    ...
    # * RUN
    # static_reconstruct(args.ws, logdir, cfg) # using uniform track dep to estimate camera's parameters
    photometric_warmup(
        args.ws, logdir, cfg
    )  # this is optional, if not set, will directly skip and return.
    scaffold_reconstruct(args.ws, logdir, cfg) # using all possible track to construct dynamic

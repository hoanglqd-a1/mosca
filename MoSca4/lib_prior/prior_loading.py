...
class Saved2D(nn.Module):
    ...
    def load_segment(self, segment_dirname="segment_mask"):
        # mark 0 as static, 1 as dynamic, 2 as unknown
        segment_mask = np.load(osp.join(self.ws, "extra", segment_dirname + ".npy"))
        # mask_list = []
        # for i in range(segment_mask.shape[0]):
        #     mask = segment_mask[i]
        #     filtered_mask = np.zeros_like(mask, dtype=np.float32) + 2
        #     static_mask = cv2.erode((mask == 0).astype(np.uint8), np.ones((4, 4), np.uint8), iterations=3)
        #     dynamic_mask = cv2.erode((mask == 1).astype(np.uint8), np.ones((4, 4), np.uint8), iterations=3)
        #     filtered_mask[static_mask > 0] = 0  # static
        #     filtered_mask[dynamic_mask > 0] = 1  # dynamic
        #     mask_list.append(filtered_mask)
        # segment_mask = np.stack(mask_list, 0)  # T,H,W
        segment_mask = torch.from_numpy(segment_mask).float()
        self.register_gradfree_buffer("segment_mask", segment_mask)
    ...
    def load_track(self, track_npz_key="spatracker", min_valid_cnt=4):
        ...
        long_track_fns += glob(osp.join(self.ws, "extra", f"*{track_npz_key}*.npz"))
        logging.info(f"Loading TAP from {long_track_fns}...")
        assert len(long_track_fns) > 0, "no TAP found!"
        track, track_mask = [], []
        for track_fn in long_track_fns:
            if "segment" not in track_fn and "uniform" not in track_fn: continue
            logging.info(f"Loading track from {track_fn}")
            # load the track data
            track_data = np.load(track_fn, allow_pickle=True)
            print(track_data["tracks"].shape, track_data["visibility"].shape)
    ...
    def normalize_depth(self, median_depth: float = 1.0):
        ...
        assert hasattr(self, "dep_mask"), "Load DEP before normalization!"

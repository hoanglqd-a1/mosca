...
def render_cam_pcl(
    ...
    for k, v in additional_key.items():
        if k not in ret and v is not None:

            v = torch.cat([v, torch.zeros_like(v).to(v.device)], -1)  # add dummy alpha channel for rasterizer

            additional_ret = rasterizer(
                means3D=means3D.float(),
                means2D=means2D.float(),
                shs=None,
                colors_precomp=v,  # TODO: support higher dimensional shading
                opacities=opacity.float(),
                scales=scales,
                rotations=rotations,
                cov3D_precomp=cov3D_precomp.float(),
            ...
            if len(additional_ret) == 2:
                rendered_buffer, _ = additional_ret
            elif len(additional_ret) == 4:
                rendered_buffer, _, _, _ = additional_ret
            else:
                raise ValueError(
                    f"Unexpected return value from rasterizer with len={len(additional_ret)}"
                )
            ret[k] = rendered_buffer[:3]

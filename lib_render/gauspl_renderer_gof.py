...
def render_cam_pcl(
    ...
    # we don't use the normal map from GOF for now
    # we will rasterize the normal map from the gaussian's normal buffer

    if normal_buffer is not None:
        extra_render, radii = rasterizer(
            means3D=means3D.float(),
            means2D=means2D.float(),
            shs=None,
            colors_precomp=normal_buffer.float(),
            opacities=opacity.float(),
            # scales=scales,
            # rotations=rotations,
            # cov3D_precomp=cov3D_precomp.float(), # ! debug
        ...
        normal = extra_render[:3]

        # normalize the normal_map

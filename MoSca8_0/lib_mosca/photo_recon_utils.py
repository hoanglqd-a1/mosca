...
def apply_gs_control(
    ...
    for render_dict in render_list:
        ...
        if ref_time_mask is not None:
            assert isinstance(model, DynSCFGaussian)
            new_grad = torch.zeros((model.N, *grad.shape[1:]), device=grad.device, dtype=grad.dtype)
            new_radii = torch.zeros((model.N, *radii.shape[1:]), device=radii.device, dtype=radii.dtype)
            new_visib = torch.zeros((model.N, *visib.shape[1:]), device=visib.device, dtype=visib.dtype)
            new_grad[ref_time_mask] = grad
            new_radii[ref_time_mask] = radii
            new_visib[ref_time_mask] = visib
            grad = new_grad
            radii = new_radii
            visib = new_visib
        if record_flag:

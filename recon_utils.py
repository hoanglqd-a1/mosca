...
def setup_recon_ws(ws, fit_cfg, logdir="logs", reuse_log_flag=False):
    ...
    root_folder = osp.basename(osp.dirname(osp.abspath(__file__)))
    if (reuse_log_flag):
        log_dirs = sorted([logdir for logdir in os.listdir(osp.join(ws, logdir)) if logdir.startswith(root_folder)])
        if len(log_dirs) == 0:
            log_dirs = sorted(os.listdir(osp.join(ws, logdir)))
        return osp.join(ws, logdir, log_dirs[-1])
    # get mosca version name
    # get datetime
    now = datetime.now()
    dt_string = now.strftime("%Y%m%d_%H%M%S")
    name = getattr(fit_cfg, "exp_name", "default")
    name = f"{root_folder}_{name}_{GS_BACKEND.lower()}_{dt_string}"
...
def update_s2d_track_identification(
    ...
    elif hasattr(s2d, "mask"):
        mask = s2d.mask.clone()
        with torch.no_grad():
            epi_error_list = query_buffers_by_track(
                mask[..., None], s2d.track, s2d.track_mask
            )
            epi_error_list = epi_error_list.squeeze(-1).cpu()
    else:
    ...
    epi_track_static_selection, epi_track_dynamic_selection = identify_tracks(
        epi_error_list, mask_th, static_cnt=1, dynamic_cnt=dyn_id_cnt
    ) if hasattr(s2d, "mask") else identify_tracks(epi_error_list, epi_th, static_cnt=1, dynamic_cnt=dyn_id_cnt)

...
from lib_mosca.misc import seed_everything, configure_logging
...
from lib_mosca.light import create_trainable_env_rnd
from lib_mosca.deform_network import DeformEnvModel
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    print(f"After track identification, get {s2d.static_track_mask.sum()} static tracks and {s2d.dynamic_track_mask.sum()} dynamic tracks")
    ...
    per_frame_curve_rgb = curve_rgb.clone()
    ...
    # idea: track of specular surface is usually not stable
    # so we identify track of reflectance surface and set them as low confidence and the others as high confident
    # we smooth these low confidence track with neighboring high confidence track
    # note: a track can be confident in some frame and not confident in other frame, because the specularity can be view dependent
    # so we set the confidence in frame level, not track level

    low_confidence_mask = torch.zeros_like(curve_mask, dtype=torch.bool)
    # 1 * track with high color shift is likely to be on specular surface, we set them as low confidence
    color_shift = torch.norm(per_frame_curve_rgb[1:] - per_frame_curve_rgb[:-1], dim=-1)
    low_confidence_th = 0.2
    low_confidence_mask = color_shift > low_confidence_th
    # for robustness, we also set the neighboring frames as low confidence
    shift_left = torch.cat([torch.zeros_like(low_confidence_mask[:1]), low_confidence_mask[:-1]], dim=0)
    shift_right = torch.cat([low_confidence_mask[1:], torch.zeros_like(low_confidence_mask[-1:])], dim=0)
    low_confidence_mask = low_confidence_mask | shift_left | shift_right
    low_confidence_mask = torch.cat([low_confidence_mask[:1], low_confidence_mask], dim=0)  # the first frame we can not compute color shift, so set it as low 

    # 2 * track with unstable motion pattern is also likely to be on specular surface, we set them as low confidence
    velocity = curve_xyz[:, 1:] - curve_xyz[:, :-1]
    acceleration = velocity[:, 1:] - velocity[:, :-1]
    acceleration_norm = torch.norm(acceleration, dim=-1)
    print(acceleration_norm.shape, curve_xyz.shape, curve_mask.shape)
    acceleration_mean = acceleration_norm.mean().item()
    acceleration_std = acceleration_norm.std().item()
    high_acceleration_mask = acceleration_norm > (acceleration_mean + 2 * acceleration_std)
    low_confidence_motion_mask = torch.zeros_like(curve_mask, dtype=torch.bool)
    low_confidence_motion_mask[:, :-2] |= high_acceleration_mask
    low_confidence_motion_mask[:, 1:-1] |= high_acceleration_mask
    low_confidence_motion_mask[:, 2:] |= high_acceleration_mask

    # debug
    low_confidence_track = low_confidence_motion_mask.sum(dim=0) > 10
    print(f"Low confidence track due to motion: {low_confidence_track.sum()} / {len(low_confidence_track)}, with mean acceleration {acceleration_norm.mean().item():.3f} and max acceleration {acceleration_norm.max().item():.3f}")
    low_confidence_mask = low_confidence_mask | low_confidence_motion_mask

    low_confidence_track = low_confidence_mask.sum(dim=0) > 50
    low_confidence_mask |= low_confidence_track.unsqueeze(0)
    ...
    low_confidence_mask = low_confidence_mask[:, sampled_inds]
    ...
    torch.save(scaffold.state_dict(), osp.join(log_path, "mosca", "pre_mosca.pth"))
    torch.save(node_rgb, osp.join(log_path, "mosca", "curve_rgb.pth"))
    return s2d
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    brdf_envmap_res = 128
    brdf_mlp = create_trainable_env_rnd(brdf_envmap_res, scale=0.0, bias=0.8)

    t_multires = 6
    deform_env = DeformEnvModel(t_multires=t_multires)
    ...
    curve_rgb = torch.load(osp.join(log_path, "mosca", "curve_rgb.pth")).to(device)
    cams: MonocularCameras = MonocularCameras.load_from_ckpt(
        torch.load(osp.join(log_path, "bundle", "bundle_cams.pth"))
    ).to(device)
    scaffold = MoSca.load_from_ckpt(
        torch.load(osp.join(log_path, "mosca", "pre_mosca.pth"))
    ).to(device)
    ...
    photo_solver.identify_fg_mask_by_segment_mask(
        s2d, "gs_model_construct_fg_mask.mp4"
    )
    ...
    if GS_BACKEND in ["gof"]:
        ...
        # s2d = s2d.load_normal()
        # viz_fn = osp.join(photo_solver.viz_dir, "gs_model_construct_normal.mp4")
        # viz_frames = (
        #     ((-s2d.nrm + 1) / 2.0 * 255).detach().cpu().numpy().astype(np.uint8)
        # )
    ...
    logging.info(f"Memory allocated before photometric fit:\n {torch.cuda.memory_allocated(device)}")
    photo_solver.photometric_fit(
        s2d=s2d,
        ...
        lambda_vel_xyz_reg=getattr(fit_cfg, "photo_lambda_vel_xyz_reg", 10.0),
        lambda_vel_rot_reg=getattr(fit_cfg, "photo_lambda_vel_rot_reg", 10.0),
        lambda_acc_rot_reg=getattr(fit_cfg, "photo_lambda_acc_rot_reg", 10.0),
        lambda_acc_xyz_reg=getattr(fit_cfg, "photo_lambda_acc_xyz_reg", 10.0),
        ...
        ##################################################
        # viz
        viz_skip_t=1 if cams.T < 120 else max(1, cams.T // 200),
        ...
        # reflectance_map=getattr(s2d, "mask", None)
    )
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reuse_log", action="store_true", help="skip log")
    args, unknown = parser.parse_known_args()

    cfg = OmegaConf.load(args.cfg)
    cli_cfg = OmegaConf.from_dotlist([arg.lstrip("--") for arg in unknown])
    cfg = OmegaConf.merge(cfg, cli_cfg)
    reuse_log_flag = args.reuse_log

    logdir = setup_recon_ws(args.ws, fit_cfg=cfg, reuse_log_flag=reuse_log_flag)

    if not torch.cuda.is_available():
        with open(osp.join(logdir, "error.txt"), "w") as f:
            f.write("No CUDA available")

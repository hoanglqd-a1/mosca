...
def render_test_tto(
    ...
    for i in tqdm(range(len(test_camera_tid))):
        ...
        with torch.no_grad():
            gs5 = [list(s_model()), list(d_model(working_t))]  # ! this does not change
            if c_model:
                s_sph = gs5[0][-1]
                s_xyz = gs5[0][0]
                _, cam_pos = cams.Rt_wc(working_t)
                ...
                direction = F.normalize(direction, dim=-1)
                gs5[1][-1] = c_model(torch.cat([d_sph, direction, d_xyz], -1).to(d_sph.device))
...
def test_main(
    ...
    c_model = ColorModule.load_from_checkpoint(
        torch.load(osp.join(saved_dir, "photometric_color.pth"))
    ).to(device)

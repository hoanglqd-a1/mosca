...
def viz2d_total_video(
    ...
    # viz the flow
    if d_model is not None and False:
    ...
    __video_save__(viz_vid_fn[:-4] + "_train.mp4", [f[::subsample, ::subsample, :] for f in frame_list])
    ...
    __video_save__(viz_vid_fn[:-4] + "_train2.mp4", [f[::subsample, ::subsample, :] for f in frame_list])
...
def viz2d_one_frame(
    ...
    view_pos = T_cw[:3, 3].repeat(N, 1)
    
    render_dict = render(gs5, cams.default_H, cams.default_W, cams.default_K, T_cw, 
                         brdf=brdf, deform_env=deform_env, view_pos=view_pos, s_model=s_model, d_model=d_model, view_ind=view_ind
                         )
...
def viz2d_single_video(
    ...
    if d_model is not None:
        ...
        frame_list_list.append(flow_frame_list1)
        flow_frame_list2, choice = viz2d_flow_video(
            start_from,
            end_at,
            cams,
        ...
        frame_list_list.append(flow_frame_list2)
        flow_frame_list3, choice = viz2d_flow_video(
            start_from,
            end_at,
            cams,
        ...
        frame_list_list.append(flow_frame_list3)
    ...
    #         [
    #             f[::subsample, ::subsample, :]
    #             for f in frame_list[cur : cur + max_num_frames]
    #         ],
    #     )
    #     cnt += 1
    #     cur += max_num_frames
    for i, frame_list in enumerate(frame_list_list):
        __video_save__(
            viz_vid_fn[:-4] + f"_{i}.mp4",
            [f[::subsample, ::subsample, :] for f in frame_list],
        )
    return
...
def viz3d_total_video(
    ...
    logging.info(f"Temporally not using this function, it causes CPU overload")
    return

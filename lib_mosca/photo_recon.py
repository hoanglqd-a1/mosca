...
from util import get_normal, reflect
from light import EnvironmentLight
from deform_network import DeformEnvModel
...
class DynReconstructionSolver:
    ...
    def identify_fg_mask_by_segment_mask(
        self, s2d: Saved2D, viz_fname=None
    ):
        assert hasattr(s2d, "mask"), "The s2d does not have segment mask for fg/bg identification"
        fg_mask = s2d.mask.clone().cpu().bool()
        s2d.register_2d_identification(
            static_2d_mask=~fg_mask, dynamic_2d_mask=fg_mask
        )

        if viz_fname is not None:
            viz_rgb = s2d.rgb.clone().cpu()
            viz_fg_mask = fg_mask * s2d.dep_mask.to(fg_mask)
            viz_rgb = viz_rgb * viz_fg_mask.float()[..., None] + viz_rgb * 0.1 * (
                1 - viz_fg_mask.float()[..., None]
            )
            imageio.mimsave(osp.join(self.viz_dir, viz_fname), viz_rgb.numpy())
        return fg_mask
    ...
    def photometric_fit(
        ...
        # stage
        stage_lighting_only=2000,
        stage_total=4000,

        lambda_residual_normal=0.02, # encourage the residual normal to be small, for better convergence and less noise
        lambda_diffuse=0.0, # encourage the diffuse render to be close to the gt, to help the lighting estimation and convergence

        reflectance_map=None, # the reflectance_map is just a mask that indicates which pixel is reflective
                              # since it is very noisy
                              # we should not supervised the render map to the gt rgb in the early stage,
                              # but instead, we should supervise the physical render result (diffuse + specular) in later stage
    ):
        ...
        optimizer_brdf = torch.optim.Adam([
            {"name": "brdf", "params": brdf_mlp.parameters()},
        ], lr=2e-2)
        optimizer_deform_env = torch.optim.Adam([
            {"name": "deform", "params": deform_env.deform.parameters()},
        ], lr=1.6e-4)
        ...
        if len(cam_param_list) > 0:
            ...
            optimizer_cam = None
        if use_decay:
            gs_scheduling_func_dict, cam_scheduling_func_dict, brdf_scheduling_dict, deform_scheduling_dict = (
        ...
        if d_flag:
            # ! for now the group rendering only works for dynamic joint mode
        ...
        view_ind_list_all = np.arange(cams.T).tolist()
        nth_batch = 0
        ...
        for step in tqdm(range(total_steps)):
            if step % 200 == 0:
                logging.info(
                    ...
                    f"Number of static gaussians: {s_model.get_x.shape[0]}, "
                    f"Number of dynamic gaussians: {d_model.get_xyz().shape[0] if d_flag else 0}, "
                    f"Memory allocated: {torch.cuda.memory_allocated(self.device) / 1024 ** 2:.2f} MB"
                )

            start_ind = nth_batch * n_frame
            end_ind = min(start_ind + n_frame, cams.T)

            nth_batch = (nth_batch + 1) % ((cams.T + n_frame - 1) // n_frame)
            if nth_batch == 0:
                np.random.shuffle(view_ind_list_all)
            ...
            lighting_optimize_flag = step >= stage_lighting_only and step < stage_total
            ...
            corr_exe_flag = (
                ...
                and not lighting_optimize_flag
            )

            sh_flag = step < stage_lighting_only
            s_model.set_sh_flag(sh_flag)
            if d_flag:
                d_model.set_sh_flag(sh_flag)
            ...
            if d_flag:
                ...
                if step % topo_update_feq == 0:
                    d_model.scf.update_topology()
            optimizer_brdf.zero_grad()
            optimizer_deform_env.zero_grad()

            # if step >= stage_lighting_only and brdf_mlp is not None:
            #     brdf_mlp.clamp_(min=0.0, max=1.0)
            ...
            if step > decay_start:
                ...
                if brdf_mlp is not None:
                    for k, v in brdf_scheduling_dict.items():
                        update_learning_rate(v(step), k, optimizer_brdf)
                if deform_env is not None:
                    for k, v in deform_scheduling_dict.items():
                        update_learning_rate(v(step), k, optimizer_deform_env)

            view_ind_list = view_ind_list_all[start_ind:end_ind]
            if corr_exe_flag:
            ...
            loss_mask = 0.0
            loss_track = 0.0
            loss_residual_normal = torch.tensor(0.0).to(s_model.device)
            loss_diffuse = torch.tensor(0.0).to(s_model.device)
            ...
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                ...
                if step >= stage_lighting_only and step < stage_total:
                    # only optimize lighting, so we detach the gs and only render the shading
                    # but we still need the gradient for the diffuse color
                    gs5 = [[it.detach() if i != 4 else it for i, it in enumerate(gs)] for gs in gs5]

                N = s_model.N + (d_model.N if d_flag else 0)
                view_pos = cams.t_wc[view_ind].repeat(N, 1).detach()
                ...
                render_dict_diffuse = None
                if step >= stage_lighting_only:
                    render_dict_diffuse = render_dict if lambda_diffuse > 0.0 else None
                    render_dict = render(
                        gs5, 
                        s2d.H,
                        s2d.W,
                        ...
                        view_pos=view_pos,
                        s_model=s_model,
                        d_model=d_model if d_flag else None,
                        detach_diffuse=(stage_lighting_only <= step < stage_lighting_only + 500),
                        deform_env=deform_env if step >= stage_lighting_only + 500 else None,
                        view_ind=view_ind,
                    )

                if step in [stage_lighting_only - 1] + list(range(stage_lighting_only, total_steps, 500)) + [stage_total - 1]:
                    # viz the render result
                    viz_rgb = render_dict["rgb"].detach().cpu().numpy() * 255.0
                    viz_rgb = viz_rgb.astype(np.uint8)
                    viz_rgb = viz_rgb.transpose(1, 2, 0)
                    imageio.imwrite(osp.join(self.viz_dir, f"render_debug_{step}.png"), viz_rgb)
                render_dict_list.append(render_dict)

                # compute losses
                rgb_sup_mask = s2d.get_mask_by_key(sup_mask_type)[view_ind]
                if reflectance_map is not None and step < stage_lighting_only:
                    rgb_sup_mask = rgb_sup_mask * (1 - reflectance_map[view_ind].to(torch.int))
                _l_rgb, _, _, _ = compute_rgb_loss(
                    s2d.rgb[view_ind].detach().clone(), render_dict, rgb_sup_mask
                )
                _l_diffuse = 0.0
                if render_dict_diffuse is not None:
                    rgb_sup_mask_diffuse = rgb_sup_mask * (1 - reflectance_map[view_ind].to(torch.int)) if reflectance_map is not None else rgb_sup_mask
                    _l_diffuse, _, _, _ = compute_rgb_loss(
                        s2d.rgb[view_ind].detach().clone(), render_dict_diffuse, rgb_sup_mask_diffuse
                    )
                    _l_diffuse = _l_diffuse
                ...
                with torch.no_grad():
                    ...
                    loss_diffuse = loss_diffuse + _l_diffuse

                _loss = _loss + _l_rgb * lambda_rgb + _l_dep * lambda_dep + _loss_track * lambda_track + _l_diffuse * lambda_diffuse
                
                # * GOF normal and regularization
                if GS_BACKEND in ["gof"] and lambda_normal > 0.0:
                    ...
                    _loss = _loss + _l_nrm * lambda_normal + _l_reg_nrm * lambda_depth_normal + _l_reg_distortion * lambda_distortion
                else:
                ...
                if d_flag and lambda_mask > 0.0:
                    ...
                    _loss = _loss + l_mask * lambda_mask
                else:
                    loss_mask = torch.zeros_like(loss_rgb)
                # call _loss.backward() in the end of this loop to get the grad for each view, and then do the optimization step after this loop
                _loss = _loss / len(view_ind_list)  # average the loss over the views in this batch
                _loss.backward()
            ...
            loss_residual_normal = s_model.compute_residual_normal_loss()
            if d_flag:
                loss_residual_normal = loss_residual_normal + d_model.compute_residual_normal_loss()

            if lighting_optimize_flag:
                loss_arap_coord = torch.zeros_like(loss_rgb)
                loss_arap_len = torch.zeros_like(loss_rgb)
                loss_vel_xyz_reg = torch.zeros_like(loss_rgb)
                loss_vel_rot_reg = torch.zeros_like(loss_rgb)
                loss_acc_xyz_reg = torch.zeros_like(loss_rgb)
                loss_acc_rot_reg = torch.zeros_like(loss_rgb)
                loss_small_w = torch.zeros_like(loss_rgb)
            ...
            loss = (
                ...
                + loss_residual_normal * lambda_residual_normal
                + loss_diffuse * lambda_diffuse
            )

            loss.backward()

            if lighting_optimize_flag:
                assert s_model._xyz.grad is None, "In lighting only stage, the geometry should not have gradient"
                assert d_model._xyz.grad is None, "In lighting only stage, the geometry should not have gradient"
                assert d_model.scf._node_xyz.grad is None, "In lighting only stage, the geometry should not have gradient"

            if step % 200 == 0:
                logging.info(
                    f"Step {step}, loss: {loss.item():.4f}, "
                    f"loss_rgb: {loss_rgb.item():.4f}, "
                    f"loss_dep: {loss_dep.item():.4f}, "
                    f"loss_nrm: {loss_nrm.item():.4f}, "
                    f"loss_mask: {loss_mask.item():.4f}, "
                    f"loss_dep_nrm_reg: {loss_dep_nrm_reg.item():.4f}, "
                    f"loss_distortion_reg: {loss_distortion_reg.item():.4f}, "
                    f"loss_arap_coord: {loss_arap_coord.item():.4f}, "
                    f"loss_arap_len: {loss_arap_len.item():.4f}, "
                    f"loss_vel_xyz_reg: {loss_vel_xyz_reg.item():.4f}, "
                    f"loss_vel_rot_reg: {loss_vel_rot_reg.item():.4f}, "
                    ...
                    f"loss_residual_normal: {loss_residual_normal.item():.4f}, "
                    f"loss_diffuse: {loss_diffuse.item():.4f}, "
                )

                self.eval_all_frames(
                    s2d,
                    cams,
                    sup_mask_type,
                    step,
                    ...
                    d_model,
                    brdf_mlp=brdf_mlp,
                    deform_env=deform_env,
                    mpsnr_list=mpsnr_list,
                )
            ...
            if brdf_mlp is not None:
                optimizer_brdf.step()
            if deform_env is not None:
                optimizer_deform_env.step()


            # d_model to s_model transfer [1] copy the d_gs5
            dynamic_to_static_transfer_flag = step in photo_s2d_trans_steps and d_flag and not lighting_optimize_flag
            ...
            if (
                ...
                if corr_exe_flag and step > dyn_node_densify_record_start_steps and not lighting_optimize_flag:
            ...
            if d_flag and step in dyn_node_densify_steps and not lighting_optimize_flag:
            ...
            # error grow
            if d_flag and step in dyn_error_grow_steps and not lighting_optimize_flag:
                error_grow_dyn_model(
                    s2d,
                    cams,
                    s_model,
            ...
            if d_flag and step in dyn_scf_prune_steps and not lighting_optimize_flag:
                d_model.prune_nodes(
                    optimizer_dynamic,
            ...
            viz_steps = [3999, stage_lighting_only - 1, stage_total - 1]
            viz_flag = step in viz_steps
            if viz_flag:
                if d_flag:
                    ...
                    viz2d_single_video(
                        viz_vid_fn=osp.join(self.viz_dir, f"{phase_name}_2dviz.mp4"),
                        s2d=s2d,
                        start_from=0,
                        end_at=cams.T - 1,
                        skip_t=viz_skip_t,
                ...
                viz2d_total_video(
                    ...
                    mask_type=sup_mask_type,
                    move_around_angle_deg=viz_move_angle_deg,
                    brdf=brdf_mlp if step >= stage_lighting_only else None,
                    deform_env=deform_env if step >= stage_lighting_only + 500 else None,
                )
        ...
        if brdf_mlp is not None:
            torch.save(
                brdf_mlp.state_dict(),
                osp.join(self.log_dir, f"{phase_name}_brdf_mlp.pth"),
            )
        if deform_env is not None:
            torch.save(deform_env.deform.state_dict(), osp.join(self.log_dir, f"{phase_name}_deform_env.pth"))
        ...
        if mpsnr_list is not None:
            plt.plot(mpsnr_list)
            plt.title(f"MPSNR: {mpsnr_list[-1]:.2f} dB")
            plt.savefig(osp.join(self.viz_dir, f"{phase_name}_mpsnr.jpg"))
            plt.close()
        ...
        if d_flag:
            ...
            if lambda_mask > 0.0:
                ...
                os.mkdir(osp.join(self.log_dir, "viz"))
                viz2d_total_video(
                ...
                viz2d_single_video(
                    viz_vid_fn=osp.join(self.log_dir, "viz", f"{phase_name}_2dviz.mp4"),
                    s2d=s2d,
                    start_from=0,
                    end_at=cams.T - 1,
                    skip_t=viz_skip_t,
    ...
    def eval_all_frames(
        ...
        with torch.no_grad():
            ...
            for tid in tqdm(range(cams.T)):
                ...
                N = s_model.N + (d_model.N if d_model is not None else 0)
                view_pos = cams.t_wc[tid].repeat(N, 1).detach()
                ...
                rgb_sup_mask = s2d.get_mask_by_key(sup_mask_type)[tid]
                _l_rgb, _, _, _ = compute_rgb_loss(
                    s2d.rgb[tid].detach().clone(), render_dict, rgb_sup_mask
                )
                _psnr = compute_psnr(
                    render_dict["rgb"].permute(1, 2, 0).detach().cpu().numpy(),
                    s2d.rgb[tid].detach().clone().cpu().numpy()
                )

                dep_sup_mask = rgb_sup_mask * s2d.dep_mask[tid]
                _nrm, _, _, _ = compute_normal_loss(
                    s2d.nrm[tid].detach().clone(), render_dict, dep_sup_mask
                )

                loss_rgb = loss_rgb + _l_rgb
                psnr = psnr + _psnr
                loss_nrm = loss_nrm + _nrm

                if max_loss_rgb < 0 or _l_rgb.item() > max_loss_rgb:
                    max_loss_rgb = _l_rgb.item()
                if min_loss_rgb < 0 or _l_rgb.item() < min_loss_rgb:
                    min_loss_rgb = _l_rgb.item()

            loss_rgb = loss_rgb / cams.T
            psnr = psnr / cams.T
            loss_nrm = loss_nrm / cams.T

            logging.info(f"Mean loss rgb {loss_rgb.item()}")
            logging.info(f"Mean psnr {psnr}")
            logging.info(f"Mean loss nrm {loss_nrm.item()}")
            logging.info(f"Max loss rgb {max_loss_rgb}, Min loss rgb {min_loss_rgb}")

            if mpsnr_list is not None:
                mpsnr_list.append(psnr)
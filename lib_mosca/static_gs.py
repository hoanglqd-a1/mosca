...
class StaticGaussian(nn.Module):
    def __init__(
        ...
        self.specular = nn.Parameter(torch.zeros(self.N, 3))
        self.roughness = nn.Parameter(torch.full((self.N, 1), default_roughness))
        self.residual_normal = nn.Parameter(torch.zeros(self.N, 3))
        ...
        self.register_buffer("diffuse_gradient_accum", torch.zeros(self.N).float())
        self.register_buffer("diffuse_gradient_denom", torch.zeros(self.N).long())
    ...
    def _densification_postprocess(
        ...
        self.diffuse_gradient_accum = torch.zeros(self._xyz.shape[0], device=self.device)
        self.diffuse_gradient_denom = torch.zeros(self._xyz.shape[0], device=self.device)
    ...
    def clean_gs_control_record(self):
        ...
        self.diffuse_gradient_accum = torch.zeros_like(self._xyz[:, 0])
        self.diffuse_gradient_denom = torch.zeros_like(self._xyz[:, 0])
    ...
    def append_gs(
        ...
        new_roughness = torch.full((len(new_mu), 1), self.default_roughness).to(self.device) if new_roughness is None else new_roughness.detach().clone()
        new_specular = torch.zeros((len(new_mu), 3)).to(self.device) if new_specular is None else new_specular.detach().clone()
        new_residual_normal = torch.zeros((len(new_mu), 3)).to(self.device) if new_residual_normal is None else new_residual_normal.detach().clone()
    ...
    def load(self, ckpt):
        ...
        self.diffuse_gradient_accum = torch.as_tensor(
            ckpt["diffuse_gradient_accum"], dtype=torch.float32
        )
        self.diffuse_gradient_denom = torch.as_tensor(
            ckpt["diffuse_gradient_denom"], dtype=torch.int64
        )
        # load others

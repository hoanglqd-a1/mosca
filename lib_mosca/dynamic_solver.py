...
def get_dynamic_curves(
    ...
    if track.shape[-1] == 3:
        ...
        if refilter_2d_track_flag and not return_all_curves:
            ...
            print(f"O3d outlier ratio {(~inlier_mask).float().mean()*100.0:.2f}%")
            ...
            print(f"shaking outlier ratio {(~inlier_mask).float().mean()*100.0:.2f}%")
...
def geometry_scf_init(
    ...
    for step in tqdm(range(total_steps)):
        ...
        loss_small_corr = diff_to_solid_xyz[solid_mask & ~low_confidence_mask].sum()  # ! use sum
        ...
        loss.backward()
        if hard_fix_valid:
            scf.mask_xyz_grad(~(solid_mask) | low_confidence_mask)
        optimizer.step()

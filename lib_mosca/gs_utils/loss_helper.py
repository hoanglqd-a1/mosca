...
def compute_shortest_axis_loss(scale):
    # we assume that the z-axis is the shortest axis
    # so we penalize gaussians with z-scale larger than the shortest axis other than the z-axis

    z_scale = scale[:, 2]
    shortest_scale = torch.min(scale[:, :2], dim=-1).values # N
    mask = z_scale > shortest_scale

    loss = torch.mean(torch.abs(z_scale[mask] - shortest_scale[mask].detach())) if mask.sum() > 0 else torch.tensor(0.0, device=scale.device)

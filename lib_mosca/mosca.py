...
class MoSca(nn.Module):
    ...
    @torch.no_grad()
    def soft_mask_xyz_grad(self, mask, factor=0.2):
        # for init stage maintain the observed xyz unchanged, but not hard mask, instead with a factor to slow down the gradient
        assert mask.shape == self._node_xyz.shape[:2]
        assert self._node_xyz.grad is not None
        mask = mask.to(self._node_xyz)
        mask = mask + (1 - mask) * factor  # if mask=1, then 1, if mask=0, then factor
        self._node_xyz.grad = self._node_xyz.grad * mask[..., None]
        return

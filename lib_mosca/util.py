...
def get_minimum_axis(scales, rotations):
    # scales: [N, 3], rotations: [N, 3, 3]
    sorted_idx = torch.argsort(scales, descending=False, dim=-1)
    R_sorted = torch.gather(rotations, dim=2, index=sorted_idx[:,None,:].repeat(1, 3, 1)).squeeze()
    return R_sorted[:,:,0]
...
def get_normal(scale, rotation, view_dir, residual_normal):
    shortest_axis = get_minimum_axis(scale, rotation)
    if residual_normal is not None:
        shortest_axis = F.normalize(shortest_axis + residual_normal, dim=-1)
    normal = F.normalize(flip_align_view(shortest_axis, view_dir.detach()), dim=-1)
    return normal

def reflect(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return 2*torch.sum(x * n, dim=-1, keepdim=True) * n - x

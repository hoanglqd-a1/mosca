...
class DynSCFGaussian(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("sph_rest_dim", torch.tensor(sph_rest_dim))
        self._features_dc = nn.Parameter(torch.zeros(init_N, 3))  # N,3
        # we use the pbr rendering to represent view-depedent color, thus we remove the higher sph component, to save the memory
        ...
        self.specular = nn.Parameter(torch.zeros(init_N, 3))
        self.roughness = nn.Parameter(torch.full((init_N, 1), default_roughness))
        self.residual_normal = nn.Parameter(torch.zeros(init_N, 3))
        ...
        self.register_buffer("diffuse_gradient_accum", torch.zeros(init_N).float())
        self.register_buffer("diffuse_gradient_denom", torch.zeros(init_N).long())
        ...
        self.register_buffer("sh_flag", torch.tensor(True))
        ...
        self.register_buffer("default_roughness", torch.tensor(default_roughness).float())
    ...
    def get_specular(self):
        return torch.sigmoid(self.specular)

    @property
    def get_roughness(self):
        return torch.sigmoid(self.roughness)

    @property
    def get_residual_normal(self):
        return self.residual_normal
    ...
    def get_c(self):
        ...
        return torch.cat([self._features_dc, torch.zeros(self.N, self.sph_rest_dim).to(self.device)], dim=-1)
    ...
    def get_optimizable_list(
        ...
        if lr_r is not None:
            l.append(
                {"params": [self.roughness], "lr": lr_r, "name": "roughness"}
            )
        if lr_spec is not None:
            l.append(
                {"params": [self.specular], "lr": lr_spec, "name": "specular"}
            )
        if lr_residual_normal is not None:
            l.append(
                {"params": [self.residual_normal], "lr": lr_residual_normal, "name": "residual_normal"}
            )
    ...
    @torch.no_grad()
    def append_new_gs(self, optimizer, tid, mu_w, quat_w, scales, opacity, rgb, curve_rgb=None):
        ...
        new_feat_rest = torch.zeros(len(scales), self.sph_rest_dim).to(
            self.device
        )

        new_specular = torch.zeros(len(scales), 3).to(self.device)
        new_roughness = torch.full((len(scales), 1), self.default_roughness).to(self.device)
        new_residual_normal = torch.zeros(len(scales), 3).to(self.device)

        # first update the knn ind
        _, attach_ind, _ = knn_points(mu_w[None], self.scf._node_xyz[tid][None], K=1)
        if curve_rgb is not None:
            # assign node's color to the new gs instead of the passed rgb, for better initialization
            new_feat_dc = RGB2SH(curve_rgb[attach_ind[0, :, 0]])
        ...
        self._densification_postprocess(
            ...
            new_residual_normal=new_residual_normal,
        )
        # self.summary(lite=True)
        return
    ...
    def gradient_based_node_densification(
        ...
        self._densification_postprocess(
            ...
            new_roughness=self.roughness[new_gs_ind].detach().clone(),
            new_specular=self.specular[new_gs_ind].detach().clone(),
            new_residual_normal=self.residual_normal[new_gs_ind].detach().clone(),
        )
    ...
    def record_diffuse_grad(self, diffuse_grad, update_filter):
        self.diffuse_gradient_accum[update_filter] += torch.norm(
            diffuse_grad[update_filter], dim=-1, keepdim=False
        )
        self.diffuse_gradient_denom[update_filter] += 1

    def record_corr_grad(self, viewspace_point_tensor_grad, update_filter):
        # Record the gradient norm, invariant across different poses
        assert len(viewspace_point_tensor_grad) == self.N, f"{len(viewspace_point_tensor_grad)}, {self.N}"
    ...
    def _densification_postprocess(
        ...
        d = {
            ...
            "residual_normal": new_residual_normal,
        }
        ...
        self.specular = optimizable_tensors["specular"]
        self.residual_normal = optimizable_tensors["residual_normal"]
        ...
        self.diffuse_gradient_accum = torch.zeros(self.N, device=self.device)
        self.diffuse_gradient_denom = torch.zeros(self.N, device=self.device)
    ...
    def clean_gs_control_record(self):
        ...
        self.diffuse_gradient_accum = torch.zeros_like(self.diffuse_gradient_accum)
        self.diffuse_gradient_denom = torch.zeros_like(self.diffuse_gradient_denom)
        return
    ...
    def _densify_and_clone(self, optimizer, grad_norm, grad_threshold, scale_th):
        ...
        new_specular = self.specular[selected_pts_mask]
        new_residual_normal = self.residual_normal[selected_pts_mask]
    ...
    def _densify_and_split(
        ...
        new_specular = self.specular[selected_pts_mask].repeat(N, 1)
        new_residual_normal = self.residual_normal[selected_pts_mask].repeat(N, 1)
    ...
    def _prune_points(self, optimizer, mask):
        ...
        self.diffuse_gradient_accum = self.diffuse_gradient_accum[valid_points_mask]
        self.diffuse_gradient_denom = self.diffuse_gradient_denom[valid_points_mask]
    ...
    def set_sh_flag(self, sh_flag):
        self.sh_flag = torch.tensor(sh_flag).bool()
        return
    ...
    def compute_residual_normal_loss(self):
        # encourage the residual normal to be small, for better convergence and less noise
        residual_normal_loss = torch.mean(torch.norm(self.residual_normal, dim=-1))
        return residual_normal_loss

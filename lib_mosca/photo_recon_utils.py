...
class OptimCFG:
    def __init__(
        ...
        lr_brdf=0.02,
        lr_brdf_final=0.0002,

        lr_deform=0.00016,
        lr_deform_final=0.000016,
    ) -> None:
        # gs
        self.lr_p = lr_p
        self.lr_q = lr_q
        ...
        self.lr_brdf = lr_brdf
        self.lr_brdf_final = lr_brdf_final

        self.lr_deform = lr_deform
        self.lr_deform_final = lr_deform_final
        return
    ...
    def get_scheduler(self, total_steps):
        ...
        brdf_scheduling_dict = {
            "brdf": get_expon_lr_func(
                lr_init=self.lr_brdf,
                lr_final=self.lr_brdf_final,
                lr_delay_mult=0.01,  # 0.02
                max_steps=total_steps,
            ),
        }
        deform_scheduling_dict = {
            "deform": get_expon_lr_func(
                lr_init=self.lr_deform,
                lr_final=self.lr_deform_final,
                lr_delay_mult=0.01,  # 0.02
                max_steps=total_steps,
            ),
        }
        return gs_scheduling_dict, cam_scheduling_dict, brdf_scheduling_dict, deform_scheduling_dict

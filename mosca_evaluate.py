...
from lib_mosca.light import load_env
...
from lib_mosca.util import get_normal
...
from lib_mosca.util import reflect
...
def render_test(
    ...
    if s_optimal is None:
        ...
        # debug
        print(f"Using s_optimal={s_optimal} for scaling the test camera trajectory")
        aligned_test_camera_T_wi = test_camera_T_wi.detach().cpu()
        aligned_test_camera_T_wi[:, :3, 3] = aligned_test_camera_T_wi[:, :3, 3] * s_optimal
    ...
    for i in tqdm(range(len(test_camera_tid))):
        ...
        view_pos = aligned_test_camera_T_wi[working_t, :3, 3].repeat(s_model.N + (d_model.N if d_model is not None else 0), 1).detach().to(device)
        render_dict = render(
            ...
            brdf=brdf_env, view_pos=view_pos, deform_env=deform_env, s_model=s_model, d_model=d_model, view_ind=working_t
        )
...
def render_test_tto(
    ...
    for i in tqdm(range(len(test_camera_tid))):
        ...
        for _step in range(tto_steps * step_factor):
            ...
            view_pos = cams.t_wc[working_t].repeat(N, 1).detach()

            render_dict = render(
                gs5, H, W, cam_K, T_cw=T_cw, brdf=brdf_env, view_pos=view_pos, deform_env=deform_env, s_model=s_model, d_model=d_model, view_ind=working_t
                )
...
def test_main(
    ...
    bundle_data = torch.load(osp.join(saved_dir, "bundle", "bundle.pth"))
    s_optimal = bundle_data["s_optimal"]

    from lib_mosca.light import create_trainable_env_rnd
    from lib_mosca.deform_network import DeformEnvModel

    brdf_env = create_trainable_env_rnd(128, 0.0, 0.8)
    brdf_env.load_state_dict(torch.load(osp.join(saved_dir, f"photometric_brdf_mlp.pth")))
    deform_env = DeformEnvModel(6)
    deform_env.deform.load_state_dict(torch.load(osp.join(saved_dir, f"photometric_deform_env.pth")))
    ...
    if not skip_test_gen:
        for test_i in range(len(gt_testing_cam_T_wi_list)):
            ...
            video_fn = f"tto_test_cam{test_i}.mp4" if tto_flag else f"test_cam{test_i}.mp4"
            # Here we concat the ground truth frames with predicted frames and the error maps
            gt_video_fn = [f for f in os.listdir(gt_dir) if f.startswith(f"{test_i+1}_")]
            gt_video_fn.sort()
            gt_frames = [imageio.imread(osp.join(gt_dir, fn)).astype(np.float32) / 255.0 for fn in gt_video_fn]
            error_maps = [np.abs(gt - pred).max(axis=-1) for gt, pred in zip(gt_frames, frames)]
            error_maps = [
                cv.applyColorMap(
                (error * 255).astype(np.uint8), cv.COLORMAP_JET)[..., ::-1].astype(np.float32) / 255.0 
                for error in error_maps
            ]
            combined_frames = [np.concatenate((gt, pred, error), axis=1) for gt, pred, error in zip(gt_frames, frames, error_maps)]
            imageio.mimsave(osp.join(saved_dir, video_fn), combined_frames)

...
def tracker_get_weighted_query_uv(mask, confidence, fid, num=1024):
    mask = mask.float()
    H, W = mask.shape
...
def get_priority_random_queries(
    ...
    for i, t in enumerate(key_inds):
        ...
        frame = video_pt[0, t] # 3, H, W
        confidence = compute_confidence_map(frame) 
        q = tracker_get_weighted_query_uv(mask, confidence=confidence, fid=t, num=int(target_num) * 3)
        queries.append(q)
...
def get_uniform_left_region_random_queries(
    video_pt, n_pts, t_max=-1, mask_list=None, interval=1
):
    queries = []
    T = video_pt.shape[1]
    if t_max > 0:
        T = min(t_max, T)
    key_inds = [i for i in range(T) if i % interval == 0]
    for i, t in enumerate(key_inds):
        if mask_list is None:
            mask = torch.ones_like(video_pt[0, t, 0])
            target_num = n_pts / T
...
def compute_confidence_map(image: torch.Tensor):
    """
    ...
    if image.ndim == 3:
        gray = image.mean(0, keepdim=True)  # (1,H,W)
    else:
        gray = image
    ...
    Ix2 = torch.nn.functional.avg_pool2d(Ix2, 5, stride=1, padding=2)
    Iy2 = torch.nn.functional.avg_pool2d(Iy2, 5, stride=1, padding=2)
    Ixy = torch.nn.functional.avg_pool2d(Ixy, 5, stride=1, padding=2)

    trace = Ix2 + Iy2
    det = Ix2 * Iy2 - Ixy * Ixy

    corner_score = det / (trace + 1e-6)

    # confidence = corner_score.squeeze()
    # confidence = confidence.clamp(min=0)
    # confidence /= confidence.max() + 1e-8

    # return confidence

    edge_score = torch.sqrt(gx**2 + gy**2 + 1e-8)

    # grad_mag = grad_mag.squeeze()
    ...
    score = corner_score.squeeze() + edge_score.squeeze()

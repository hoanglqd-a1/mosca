...
from tracking_utils import (
    get_uniform_left_region_random_queries,
    get_priority_random_queries,
    seed_everything,
...
def cotracker_process_folder(
    ...
    for round in range(num_slice):
        if priority:
            priority_queries = get_priority_random_queries(
                video_pt, chunk_size // 2, mask_list=sample_mask_list
            )
            uniform_queries = get_uniform_random_queries(
                video_pt, chunk_size - chunk_size // 2, mask_list=sample_mask_list
            )
            # left_region_queries = get_uniform_left_region_random_queries(
            #     video_pt, chunk_size // 4, mask_list=sample_mask_list
            # )
            queries = torch.concat([priority_queries, uniform_queries], dim=1)
        else:

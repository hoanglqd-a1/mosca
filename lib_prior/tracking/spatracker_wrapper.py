...
def spatracker_process_folder(
    ...
    # in case of very long video, we create a 2x faster video to get a half-size query set
    # and then we do the inference on the new video
    # and interpolate the query back to the original video, which can significantly reduce the inference time with a little performance drop

    # if video_pt.shape[1] > 500:
    #     logging.info(
    #         f"Video length {video_pt.shape[1]} is too long, we will do a 2x faster video for query sampling to reduce the inference time!"
    #     )
    #     video_pt = video_pt[:, ::2]
    #     dep_pt = dep_pt[::2]
    #     sample_mask_list = sample_mask_list[::2]
    ...
    for round in range(num_slice):
        ...
        # here we apply the queries to the original video by interpolation, which can significantly reduce the inference time with a little performance drop, especially for long videos

        viz_list = viz_queries(queries.squeeze(0), H, W, T)
        imageio.mimsave(
            osp.join(viz_dir, f"{save_name}_r={round}_queries.mp4"), viz_list
        )

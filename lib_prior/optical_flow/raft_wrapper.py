...
def raft_process_folder(
    ...
    # load all flow and mask, and save as a video for visualization
    flow_viz_list, flow_mask_viz_list = [], []
    for vi, vj in pair_list:
        if vi >= vj: continue

        flow_fwd = np.load(os.path.join(dst_dir, f"{vi}_to_{vj}.npz"))["flow"]
        mask_fwd = np.load(os.path.join(dst_dir, f"{vi}_to_{vj}.npz"))["mask"]
        flow_viz_fwd = flow_viz.flow_to_image(flow_fwd)
        flow_viz_list.append(flow_viz_fwd)
        flow_mask_viz_list.append(mask_fwd.astype(np.uint8) * 255)

    adjacency_flow_viz_list, adjacency_flow_mask_viz_list = [], []
    adjacent_pair_list = get_neighboring_pair_list(img_name_list)
    for vi, vj in adjacent_pair_list:
        flow_fwd = np.load(os.path.join(dst_dir, f"{vi}_to_{vj}.npz"))["flow"]
        mask_fwd = np.load(os.path.join(dst_dir, f"{vi}_to_{vj}.npz"))["mask"]
        adjacency_flow_viz_fwd = flow_viz.flow_to_image(flow_fwd)
        adjacency_flow_viz_list.append(adjacency_flow_viz_fwd)
        adjacency_flow_mask_viz_list.append(mask_fwd.astype(np.uint8) * 255)
    ...
    imageio.mimsave(
        os.path.join(os.path.dirname(dst_dir), "adjacency_flow_viz.mp4"),
        adjacency_flow_viz_list,
    )
    imageio.mimsave(
        os.path.join(os.path.dirname(dst_dir), "adjacency_flow_mask_viz.mp4"),
        adjacency_flow_mask_viz_list,
    )
    print(f"RAFT flow saved to {dst_dir}")
    return

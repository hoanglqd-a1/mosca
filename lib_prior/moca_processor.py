...
class MoCaPrep:
    ...
    def __init_depth__(
        ...
        elif self.dep_mode == "depthanythingv2":
            from depth_models.depthanythingv2_wrapper import depthanythingv2_process_folder
            from depth_models.depthanythingv2_wrapper import get_depthanythingv2_model

            self.depth_model = get_depthanythingv2_model(device)
            torch.cuda.empty_cache()
            self.depth_process_func = depthanythingv2_process_folder
            self.align_to_metric = True
            logging.warning(
                f"DepthAnythingV2 is not a metric model, align to metric model"
            )
    ...
    def compute_depth(
        ...
        if save_dir.endswith("as"):
            fallback_fov = 50.349487
        elif save_dir.endswith("basin"):
            fallback_fov = 50.460285
        elif save_dir.endswith("bell"):
            fallback_fov = 49.82826
        elif save_dir.endswith("cup"):
            fallback_fov = 50.546356
        elif save_dir.endswith("plate"):
            fallback_fov = 49.985584
        elif save_dir.endswith("press"):
            fallback_fov = 49.66475
        elif save_dir.endswith("sieve"):
            fallback_fov = 51.129658
        ...
        if self.dep_mode == "metric3d":
            ...
            if K is None:
                ...
                fxfycxcy_pixel = None
                logging.warning(
                    f"Camera intrinsic K not provided for metric3d, use fallback_fov {fallback_fov} to compute fxfycxcy_pixel"
                )
            else:
                default_fov = None

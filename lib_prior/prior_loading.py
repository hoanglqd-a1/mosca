...
def filter_track(track, track_vis, dep_mask_list, min_valid_cnt=4):
    ...
    print(f"Valid check: min_cnt={min_valid_cnt}, invalid track count: {(~filter_track_mask).sum()}")
...
class Saved2D(nn.Module):
    ...
    def load_rgb(self, rgb_dirname="images"):
        ...
        if osp.exists(img_npz):
            ...
            images = [imageio.imread(osp.join(img_dir, img_fn), pilmode="RGB") for img_fn in img_fns]
    ...
    def load_dep(
        ...
        print(f"Loading depth from {dep_dir}...")
        dep_confidence = None
        if osp.isdir(dep_dir):
            ...
            try:
                dep_confidence = [
                    np.load(osp.join(dep_dir, f"{img_name}.npz"))["confidence"]
                    for img_name in self.frame_names
                ]
                dep_confidence = np.stack(dep_confidence)  # T,H,W
            except KeyError:
                logging.warning("Depth confidence not found, skipping...")
        else:
            dep_fn = dep_dir + ".npz"
        ...
        #     high_confidence_mask = dep_confidence > 0.95
        #     print(f"High confidence mask {high_confidence_mask.mean()*100:.2f}%")
        #     dep_mask = dep_mask & high_confidence_mask
        # assign
    ...
    @torch.no_grad()
    def load_mask(self, mask_dirname="masks"):
        mask_dir = osp.join(self.ws, mask_dirname)
        if not osp.exists(mask_dir):
            logging.warning(
                f"Calling 2D Mask loading, but {mask_dir} not found, usually this means the data is not available, so skip!"
            )
            return self
        mask_list = []
        for img_name in self.frame_names:
            mask_fn = osp.join(mask_dir, f"{img_name}.png") if osp.exists(osp.join(mask_dir, f"{img_name}.png")) else osp.join(mask_dir, f"{img_name}.jpg")
            mask = imageio.imread(mask_fn, as_gray=True)
            mask_list.append(mask)
        mask_list = np.stack(mask_list, 0) < 128  # T,H,W, bool
        print(f"Mask shape: {mask_list.shape}, unique values: {np.unique(mask_list)}")
        self.register_gradfree_buffer("mask", torch.from_numpy(mask_list))
        return self
    
    @torch.no_grad()
    def load_mask_2(self, mask_dirname="extra/segment_mask.npy"):
        mask_fn = osp.join(self.ws, mask_dirname)
        if not osp.exists(mask_fn):
            logging.warning(
                f"Calling 2D Mask loading, but {mask_fn} not found, usually this means the data is not available, so skip!"
            )
            return self
        mask_list = np.load(mask_fn)  # T,H,W, bool
        print(f"Mask shape: {mask_list.shape}, unique values: {np.unique(mask_list)}")
        self.register_gradfree_buffer("mask", torch.from_numpy(mask_list))
        return self
    
    @torch.no_grad()
    def load_normal(self, normal_dirname="normals/normals_npy"):
        normal_dir = osp.join(self.ws, normal_dirname)
        if not osp.exists(normal_dir):
            logging.warning(
                f"Calling 2D Normal loading, but {normal_dir} not found, usually this means the data is not available, so skip!"
            )
            return self
        normal_list = []
        for img_name in sorted(os.listdir(normal_dir)):
            if not img_name.endswith(".npy"):
                continue
            normal_fn = osp.join(normal_dir, img_name)
            normal = np.load(normal_fn).transpose(1, 2, 0)  # H,W,3
            normal_list.append(-normal)
        normal_list = np.stack(normal_list, 0)  # T,H,W,3
        self.register_gradfree_buffer("nrm", torch.from_numpy(normal_list).float().to(self.rgb.device))
        print(f"Normal shape: {normal_list.shape}")
        return self
    ...
    @torch.no_grad()
    def rescale_depth(self, dep_scale, dep_correction, residual_correction=None):
        dep_scale = dep_scale.to(self.dep.device)
        dep_correction = dep_correction.to(self.dep.device)
        if residual_correction is not None:
            residual_correction = residual_correction.to(self.dep.device)
        assert len(dep_scale) == self.T, f"dep_scale:{len(dep_scale)} vs T:{self.T}"
        assert len(dep_correction) == self.T, f"dep_correction:{len(dep_correction)} vs T:{self.T}"
        self.dep = self.dep.clone() * dep_scale[:, None, None] + dep_correction[:, None, None]
        if residual_correction is not None:
            self.dep = self.dep + torch.nn.functional.interpolate(
                residual_correction[:, None], size=(self.H, self.W), mode="bilinear", align_corners=False
            )[:, 0]
        if self.track.shape[-1] == 3:
            logging.info(f"Also align the 3D track with the depth scale")
            self.track[:, :, 2] = self.track[:, :, 2].clone() * dep_scale[:, None] + dep_correction[:, None]
            if residual_correction is not None:
                self.track[:, :, 2] = self.track[:, :, 2] + torch.nn.functional.interpolate(
                    residual_correction[:, None], size=(self.H, self.W), mode="bilinear", align_corners=False
                )[:, 0]
        return
    ...
    def rescale_perframe_depth_from_bundle(self, bundle_pth_fn=None):
        ...
        dep_correction = bundle_data["dep_correction"]
        self.rescale_depth(dep_scale, dep_correction)
        return self
    ...
    @torch.no_grad()
    def smooth_depth(self, ksize=5):
        dep = self.dep.cpu().numpy()
        dep_smooth = np.zeros_like(dep)
        for t in range(dep.shape[0]):
            dep_smooth[t] = cv2.GaussianBlur(dep[t], ksize=(ksize, ksize), sigmaX=0, sigmaY=0)
        self.register_gradfree_buffer("dep", torch.Tensor(dep_smooth))

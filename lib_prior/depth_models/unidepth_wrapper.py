...
sys.path.append(osp.join(osp.abspath(osp.dirname(__file__)), "UniDepth"))
...
@torch.no_grad()
def process(image, out_fn, model, default_fov_deg=50.349487):
    fov_deg = default_fov_deg
    H, W = image.shape[-2:]
    tanfov = np.tan(np.radians(fov_deg / 2))
    f = min(H, W) / 2 / tanfov
    K = torch.tensor([
        [f, 0, W / 2],
        [0, f, H / 2],
    ...
    camera = Pinhole(K=K).to(image.device)
    pred = model.infer(image, camera)
    dep = pred["depth"]
...
def unidepth_process_folder(
    ...
    for i in tqdm(range(len(fn_list))):
        ...
        dep = process(img.to(device), out_fn, model, default_fov_deg=default_fov_deg)
...
if __name__ == "__main__":
    ...
    image_list = [imageio.imread(osp.join(img_dir, fn)) for fn in fn_list]

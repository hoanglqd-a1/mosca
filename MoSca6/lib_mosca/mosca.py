...
class MoSca(nn.Module):
    ...
    def __init__(
        ...
        self.deformation = Deformation(feature_dim)
        self.node_feature = nn.Parameter(
            torch.zeros(self._node_xyz.shape[1], feature_dim, device=self.device), 
        )
        self.xyz_cannon = nn.Parameter(
            self._node_xyz.data[0]
        )
        self.summary()
        return
    
    def positional_encoding(self, feature, encoding_degree):
        powers = 2.**torch.arange(encoding_degree)
    ...
    def resample_node(self, resample_factor=1.0, use_mask=False, resample_ind=None):
        ...
        new_node_feature = self.node_feature[resample_ind]
        with torch.no_grad():
            ...
            self.node_feature.data = new_node_feature
    ...
    def update_node_xyz_quat(self):
        ...
        xyz_cannon = self.xyz_cannon.repeat(len(t_list), 1)
        node_cnt = self.node_feature.shape[0]
        view_idx_tensor = t_list.repeat_interleave(node_cnt)
        node_feature = self.node_feature.repeat(len(t_list), 1)
        xyz_pred = self.deformation(node_feature, view_idx_tensor.unsqueeze(1), xyz_cannon)
        ...
        assert xyz_pred.shape == self._node_xyz.shape, f"Misaligned xyz shape {xyz_pred.shape} vs {self._node_xyz.shape}"

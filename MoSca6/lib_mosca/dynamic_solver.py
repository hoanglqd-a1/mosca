...
def feature_scf_init(
    ...
    node_feature = scf.node_feature = torch.nn.Parameter(
        torch.zeros(scf._node_xyz.shape[1], scf.feature_dim)
    )

    scf = scf.to(device)
    optimizer = torch.optim.Adam(
        [
            {"params": scf.deformation.parameters(), "lr": 1e-3},
            {"params": scf.node_feature, "lr": 1e-3}
        ]
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
    ...
    batch_size = scf.node_feature.shape[0]
    with torch.no_grad():
        t_list = scf._t_list[1:]
        view_idx_tensor = t_list.repeat_interleave(batch_size).unsqueeze(1)
        xyz_gt = torch.cat([scf._node_xyz[view_idx] for view_idx in t_list], dim=0)
        xyz_cannon = scf._node_xyz[0].repeat(len(t_list), 1)

    for step in tqdm(range(total_steps)):
        optimizer.zero_grad()
        node_feature_batch = node_feature.repeat(len(t_list), 1)
        xyz_pred = deformation(node_feature_batch, view_idx_tensor, xyz_cannon)
        xyz_pred = xyz_pred
        xyz_loss = torch.nn.MSELoss()(xyz_pred, xyz_gt)
        loss = xyz_loss
        loss.backward()
        optimizer.step()
        if step > decay_start:

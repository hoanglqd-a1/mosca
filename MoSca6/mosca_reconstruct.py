...
from lib_mosca.dynamic_solver import geometry_scf_init, feature_scf_init
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    feature_scf_init(
        osp.join(log_path, "mosca"),
        scaffold,
        device,
        total_steps=8000
    )
    test_featured_scf = [scaffold.deformation(scaffold.node_feature, tid, scaffold._node_xyz[0]) for tid in range(1, cams.T)]
    viz_list = viz_list_of_colored_points_in_cam_frame(
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    logging.info(f"Memory allocated after loading s2d: {torch.cuda.memory_allocated(device) / 1024 ** 2} MB")
    ...
    logging.info(f"Memory allocated after loading cams and scaffold: {torch.cuda.memory_allocated(device) / 1024 ** 2} MB")
    ...
    logging.info(f"Memory allocated after loading photo solver: {torch.cuda.memory_allocated(device) / 1024 ** 2} MB")
    ...
    logging.info("Memory allocated after loading static model:", torch.cuda.memory_allocated(device) / 1024 ** 2, "MB")

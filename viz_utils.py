...
def viz_world_points_in_test_cam_frame(
    cams, 
    test_cams_T,
    s2d,
    s_optimal,
    ...
    with torch.no_grad():
        ...
        rel_focal = 1.0
        device = torch.device("cuda:0")
        test_cams_T[:, :3, 3] = test_cams_T[:, :3, 3] * s_optimal
        test_cams_T = test_cams_T.to(device)
        bg_color = [1.0, 1.0, 1.0]
        K = cams.K(H, W)
        for t in tqdm(range(T)):
            homo_map = cams.get_homo_coordinate_map(H, W)
            dep = s2d.dep[t]
            rgb = s2d.rgb[t]
            pts = cams.backproject(homo_map.reshape(-1, 2), dep.reshape(-1))
            world_pts = cams.trans_pts_to_world(t, pts)
            
            xyz = world_pts.to(device)
            scale = dep / (rel_focal * 0.5 * (K[0, 0] + K[1, 1])) * 2.0
            scale = scale.reshape(-1, 1).float().to(device).expand(-1, 3)
            rotation = torch.eye(3)[None].expand(len(xyz), -1, -1).to(device)
            opacity = torch.ones(len(xyz), 1).to(device)
            sh = RGB2SH(rgb.reshape(-1, 3).float().to(device))
            
            inv_test_cams_T_t = torch.linalg.inv(test_cams_T[t])
            R_cw, t_cw = inv_test_cams_T_t[:3, :3], inv_test_cams_T_t[:3, 3]
            # R_cw, t_cw = cams.Rt_cw(t)
            mu_cam = torch.einsum("ij,nj->ni", R_cw, xyz) + t_cw
            
            N = mu_cam.shape[0]
            assert scale.shape[0] == N and scale.shape[1] == 3 and scale.ndim == 2, f"Scale shape mismatch: {scale.shape} vs {N}"
            assert rotation.shape[0] == N and rotation.shape[1] == 3 and rotation.shape[2] == 3, f"Rotation shape mismatch: {rotation.shape} vs {N}"
            assert opacity.shape[0] == N and opacity.shape[1] == 1, f"Opacity shape mismatch: {opacity.shape} vs {N}"
            assert sh.shape[0] == N, f"SH shape mismatch: {sh.shape} vs {N}"
...
def viz_world_points_in_test_cam_frame_with_mask(
    cams,
    test_cams_T,
    s2d,
    s_optimal,
    static_mask,
    max_sample_static_points=100000,
):
    with torch.no_grad():
        T = cams.T
        viz_list = []
        ...
        bg_color = [1.0, 1.0, 1.0]
        K = cams.K(H, W)
        static_pts, static_pts_color, static_pts_scale = [], [], []
        for t in tqdm(range(T)):
            static_mask_t = static_mask[t].to(device).bool()
            static_homo_map = cams.get_homo_coordinate_map(H, W)[static_mask_t]
            static_dep = s2d.dep[t][static_mask_t]
            pts = cams.backproject(static_homo_map.reshape(-1, 2), static_dep.reshape(-1))
            world_pts = cams.trans_pts_to_world(t, pts)

            rgb = s2d.rgb[t][static_mask_t]

            scale = s2d.dep[t][static_mask_t] / (rel_focal * 0.5 * (K[0, 0] + K[1, 1]))
            scale = scale.reshape(-1, 1).float().to(device).expand(-1, 3)

            static_pts.append(world_pts)
            static_pts_color.append(rgb)
            static_pts_scale.append(scale)

        static_pts = torch.cat(static_pts, dim=0)
        static_pts_color = torch.cat(static_pts_color, dim=0)
        static_pts_scale = torch.cat(static_pts_scale, dim=0)

        if max_sample_static_points < len(static_pts):
            indices = torch.randperm(len(static_pts))[:max_sample_static_points]
            static_pts = static_pts[indices]
            static_pts_color = static_pts_color[indices]
            static_pts_scale = static_pts_scale[indices]

        static_pts_rotation = torch.eye(3)[None].expand(len(static_pts), -1, -1).to(device)
        static_pts_opacity = torch.ones(len(static_pts), 1).to(device)

        for t in tqdm(range(T)):
            dynamic_mask = ~static_mask[t]
            dynamic_homo_map = cams.get_homo_coordinate_map(H, W)[dynamic_mask]
            dynamic_dep = s2d.dep[t][dynamic_mask]
            dynamic_pts = cams.backproject(dynamic_homo_map.reshape(-1, 2), dynamic_dep.reshape(-1))
            world_dynamic_pts = cams.trans_pts_to_world(t, dynamic_pts)

            xyz = torch.cat([static_pts, world_dynamic_pts], dim=0)
            rgb = s2d.rgb[t][dynamic_mask]
            rgb = torch.cat([static_pts_color, rgb], dim=0)

            scale = s2d.dep[t][dynamic_mask] / (rel_focal * 0.5 * (K[0, 0] + K[1, 1]))
            scale = scale.reshape(-1, 1).float().to(device).expand(-1, 3)
            scale = torch.cat([static_pts_scale, scale], dim=0)

            rotation = torch.eye(3)[None].expand(len(world_dynamic_pts), -1, -1).to(device)
            rotation = torch.cat([static_pts_rotation, rotation], dim=0)
            opacity = torch.ones(len(world_dynamic_pts), 1).to(device)
            opacity = torch.cat([static_pts_opacity, opacity], dim=0)
            ...
            assert rotation.shape[0] == N and rotation.shape[1] == 3 and rotation.shape[2] == 3, f"Rotation shape mismatch: {rotation.shape} vs {N}"
            assert opacity.shape[0] == N and opacity.shape[1] == 1, f"Opacity shape mismatch: {opacity.shape} vs {N}"
            assert rgb.shape[0] == N and rgb.shape[1] == 3, f"RGB shape mismatch: {rgb.shape} vs {N}"

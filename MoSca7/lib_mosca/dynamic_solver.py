...
def feature_scf_init(
    ...
    batch_size = scf.xyz_cannon.shape[0]
    with torch.no_grad():
    ...
    for step in tqdm(range(total_steps)):
        optimizer.zero_grad()
        xyz_pred = deformation(scf.xyz_cannon, view_idx_tensor, all_view_flag=True, num_time_steps=len(t_list))
        xyz_loss = torch.nn.functional.mse_loss(xyz_pred, xyz_gt)
        loss = xyz_loss
        loss.backward()
        optimizer.step()
        if step > decay_start:

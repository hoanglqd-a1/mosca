...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    logging.info("Memory allocated after loading dynamic model:", torch.cuda.memory_allocated(device) / 1024 ** 2, "MB")
    with torch.no_grad():
    ...
    logging.info("Memory allocated before photometric fit:\n", torch.cuda.memory_summary(device))
    photo_solver.photometric_fit(
        s2d=s2d,
...
if __name__ == "__main__":
    ...
    logdir = setup_recon_ws(args.ws, fit_cfg=cfg, reuse_log_flag=False)

...
from bundle import (
    ...
    compute_track_epipolar_errors,
    triangulate_3Dpoints_from_tracks,
)
...
def rescale_camera_pose(
    ...
    return cams, s_optimal


@torch.no_grad()
def rescale_depth_to_camera(s2d, cams, s_track, s_track_mask, robust_alignment_jitter_th_ratio=2.0):
    """
    Compute the same closed-form optimal scale as `rescale_camera_pose` but apply
    the inverse scale to the per-frame depths in `s2d` instead of scaling camera
    translations. This keeps camera poses untouched and brings depth into the
    camera pose metric.

    Note: if cameras were originally scaled to match depth (cams.t *= s_optimal),
    then to instead scale depths to match cams we multiply depths by 1/s_optimal.
    """
    device = s2d.dep.device
    homo_list, dep_list, rgb_list = prepare_track_homo_dep_rgb_buffers(
        s2d, s_track[...,:2], s_track_mask, torch.arange(s2d.T).to(device)
    ...
    if s_optimal < 0.00001:
        ...
        s_optimal = 1.0
    logging.info(
        f"Compute optimal GT camera scale {s_optimal}; apply inverse to depths (1/{s_optimal}) to align depth to cameras"
    )

    return s2d, s_optimal
...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track", "mask"], f"{static_id_mode} not in [raft, track, mask]"
    s2d.to(device)
    ...
    elif static_id_mode == "mask":
        logging.info(f"Use the mask to mark static tracks")
        # * mark by directly using the mask, which is more strict but more robust
        segment_mask = s2d.mask.clone()
        with torch.no_grad():
            epierr_list = query_buffers_by_track(segment_mask[..., None], track, track_mask)
            epierr_list = epierr_list.squeeze(-1).cpu()

    track_static_selection, track_dynamic_selection = identify_tracks(
        epierr_list, epi_th
    ) if static_id_mode in ["raft", "track"] else identify_tracks(epierr_list, mask_th)

    print(f"Static track count: {track_static_selection.sum()}, Dynamic track count: {track_dynamic_selection.sum()}")
    logging.info(f"Static track count: {track_static_selection.sum()}, Dynamic track count: {track_dynamic_selection.sum()}")
    ...
    if gt_cam is None:
        ...
        if rescale_gt_cam_transl:
            if rescale_gt_depth:
                logging.info(
                    "Rescaling depths to match GT camera translations (keep camera poses unchanged)"
                )
                s2d, s_optimal = rescale_depth_to_camera(
                ...
                cams, s_optimal = rescale_camera_pose(
                    s2d, cams, s_track=sta_track, s_track_mask=sta_track_mask
                )

        cams.to(device)

    print(s2d.dep_mask.float().mean().item() * 100.0, "% valid depth pixels")
    dep = s2d.dep * s2d.dep_mask
    imageio.mimsave(osp.join(ws, "dep_mask.mp4"), (dep.cpu().numpy() / dep.max().item() * 255).astype(np.uint8))

    sta_track = sta_track.to(device)
    sta_track_mask = sta_track_mask.to(device)
    sta_track_dep = sta_track_dep.to(device)

    # if gt_cam is not None and False:
    #     # compute epipolar error on each track with the GT cam, remove tracks with large epi error, and only keep the tracks with small epi error for later BA, which is more robust to outliers in the GT cam
    #     logging.info(
    #         f"GT cam provided, compute the epipolar error for each track and only keep the tracks with small epi error for later BA, which is more robust to outliers in the GT cam"
    #     )
        
    #     per_track_error, pair_epi_error_list = compute_track_epipolar_errors(
    #         sta_track, sta_track_mask, cams
    #     )

    #     for i in range(10):
    #         th = float(torch.nanquantile(per_track_error, 0.1 * (i + 1)))
    #         keep_mask = (per_track_error < th) & (~torch.isnan(per_track_error)) & (~torch.isinf(per_track_error))
    #         print(f"Quantile {0.1 * (i + 1):.1f}, th={th:.2f}, keep {keep_mask.sum()}/{len(keep_mask)} tracks")
        
    #     th = min(float(torch.nanquantile(per_track_error, 0.8)), 0.2)
    #     keep_mask = (per_track_error < th) & (~torch.isnan(per_track_error)) & (~torch.isinf(per_track_error))
    #     logging.info(
    #         f"GT cam epi error filtering: keep {keep_mask.sum()}/{len(keep_mask)} tracks with epi error < {th:.2f} (90% quantile), ignore the rest for later BA"
    #     )

    #     sta_track = sta_track[:, keep_mask]
    #     sta_track_mask = sta_track_mask[:, keep_mask]
    #     sta_track_dep = sta_track_dep[:, keep_mask]

    a, b = None, None
    if gt_cam is not None:
        with torch.no_grad():
            # compute the triangulated 3D points from the tracks with the GT cam, and compute the depth error of the triangulated points, which can be used to set the robust th for later BA
            logging.info(
                f"GT cam provided, compute the triangulated 3D points from the tracks with the GT cam"
            )
            points, prj_errs, obs_count = triangulate_3Dpoints_from_tracks(
                sta_track.to(device), sta_track_mask.to(device), cams
            )

            for i in range(10):
                th = float(torch.nanquantile(prj_errs, 0.1 * (i + 1)))
                print(f"Quantile {0.1 * (i + 1):.1f}, th={th:.4f}")

            th = min(float(torch.nanquantile(prj_errs, 0.8)), 1.0, torch.sort(prj_errs)[0][min(4000, len(prj_errs) - 1)].item())
            keep_mask = (prj_errs < th) & (~torch.isnan(prj_errs)) & (~torch.isinf(prj_errs))
            logging.info(
                f"GT cam triangulation reprojection error filtering: keep {keep_mask.sum()}/{len(keep_mask)} tracks with prj error < {th:.2f}, ignore the rest for later BA"
            )
            ...
            a = torch.zeros(s2d.T, device=device)
            b = torch.zeros(s2d.T, device=device)
            error = 0.0
            for t in range(s2d.T):
                mask = sta_track_mask[t]
                if mask.sum() == 0:
                    continue

                # compute the depth of the triangulated points in this frame
                track_pts = sta_track[:, mask][t]
                track_pts_depth = sta_track_dep[:, mask][t, :, 0]
                track_pts_3d = points[mask]
                track_pts_3d_cam = cams.trans_pts_to_cam(t, track_pts_3d)
                track_pts_3d_cam_depth = track_pts_3d_cam[:, 2]

                # solve affine transform from the track depth to the triangulated depth, and compute the scale and shift
                X = torch.stack([track_pts_depth, torch.ones_like(track_pts_depth)], dim=-1)
                a_t, b_t = torch.linalg.lstsq(X, track_pts_3d_cam_depth).solution
                affined_depth = track_pts_depth * a_t + b_t
                depth_error_percent = torch.abs(affined_depth - track_pts_3d_cam_depth) / track_pts_3d_cam_depth.mean() * 100.0
                a[t] = a_t
                b[t] = b_t
                error += depth_error_percent.mean().item()

            error = error / s2d.T
            print(
                f"GT cam triangulated points have average depth error {error:.2f}% after affine transform, scale range [{a.min():.3f}, {a.max():.3f}], shift range [{b.min():.3f}, {b.max():.3f}]"
            )
    ...
    print("Viz all points shape:", viz_all_pts.shape)

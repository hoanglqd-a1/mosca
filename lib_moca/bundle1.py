...
def compute_static_ba(
    ...
    param_dep_residual = torch.zeros(cams.T, H//8, W//8).to(device)
    param_dep_residual.requires_grad_(True)
    ...
    original_dep = s2d.dep.clone().to(device)
    ...
    for step in tqdm(range(total_steps)):
        if step == switch_to_ind_step:
            ...
            if lr_dep_residual > 0:
                optim_list.append(
                    {
                        "params": [param_dep_residual],
                        "lr": lr_dep_residual,
                        "name": "dep_residual",
                    }
                )
        ...
        refined_dep = original_dep * dep_scale[:, None, None]
        if step >= depth_correction_after_step:
            refined_dep = refined_dep + param_dep_corr[:, None, None]
            dep_corr_loss = abs(param_dep_corr).mean()
        else:
            dep_corr_loss = torch.zeros_like(param_dep_corr.mean()).to(device)

        if step >= depth_correction_after_step + 500:
            refined_dep = refined_dep + torch.nn.functional.interpolate(
                param_dep_residual[:, None], size=(H, W), mode="bilinear", align_corners=False
            )[:, 0]
            dep_corr_loss = dep_corr_loss + abs(param_dep_residual).mean()
            
        scaled_depth_list = get_track_buffer(s2d, s_track, s_track_valid_mask, torch.arange(s2d.T).to(device), refined_dep)
    ...
    dep_residual = param_dep_residual.detach().clone()

    logging.info(f"Update the S2D depth scale with {dep_scale}")
    s2d.rescale_depth(dep_scale, dep_corr, dep_residual)
...
def get_track_buffer(s2d, track, track_mask, t_list, dep):
    dep_list = []
    for ind, tid in enumerate(t_list):
        _uv = track[ind]
        _int_uv, _inside_mask = round_int_coordinates(_uv, s2d.H, s2d.W)
    ...
    dep_list = torch.stack(dep_list, 0)
    dep_list[~track_mask] = -1
    return dep_list

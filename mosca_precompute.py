...
def preprocess(
    ...
    if segment_mask:
        print("Using the segment mask for dynamic track resampling, make sure the mask is in grayscale and the dynamic region is marked with white (255)")
        mask_dir = osp.join(ws, "masks")
        print(f"Loading masks from {mask_dir}")
        mask_fns = sorted(
            [it for it in os.listdir(mask_dir) if it.endswith(".png") or it.endswith(".jpg")]
        )
        print(f"Found {len(mask_fns)} masks, {len(img_fns)} images")
        mask_list = [imageio.imread(osp.join(mask_dir, it), pilmode="L") for it in mask_fns]
        assert len(mask_list) == len(img_list), f"The number of masks should be the same as the number of images, but got {len(mask_list)} masks and {len(img_list)} images"
        mask_list = [torch.from_numpy(mask < 128) for mask in mask_list]  # the dynamic region is marked with white (255)
        mask_list = torch.stack(mask_list, dim=0)

        sample_mask = mask_list
    ...
    moca_processor.compute_tap(
        ...
        # n_track=8192 * 3,
        n_track=getattr(pre_cfg, "n_track_dynamic", 8192 * 2 - 1024),
        img_list=img_list,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask", action="store_true", help="whether to use the segment mask for dynamic track resampling")
